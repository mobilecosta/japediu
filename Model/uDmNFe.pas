...
uses
  ...
  ACBrMail, vcl.Menus, ACBrIntegrador, ACBrNFeDANFEFR;
...
type
  ...
  public
    ...
    procedure impressoraNfceFast(Tipo : string);
...
uses Udados, ufrmStatus, uDMEstoque, uAPITelegramMensagem;
...
//      raise Exception.Create('Erro:' + sLineBreak +
//        ACBrNFe.WebServices.EnvEvento.XMotivo);
//    LogTelegramBot('dmNFe.Cancelar NFE ', 'Erro:' + sLineBreak +
//        ACBrNFe.WebServices.EnvEvento.XMotivo, 'Retaguarda');
  end;
...
//  if dados.qryConfigUF.Value = 'CE' then
//  begin
...
procedure TdmNFe.impressoraNfceFast(Tipo: string);
begin
 ACBrNFe.DANFE := fast;
 ACBrNFe.DANFE.PathPDF := Dados.qryConfigPATHPDF.Value;
 ACBrNFe.DANFE.NumCopias := 1;
 if Dados.qryConfigNCOPIAS.AsInteger > 1 then
   ACBrNFe.DANFE.NumCopias := Dados.qryConfigNCOPIAS.Value;
 ACBrNFe.DANFE.Logo := Dados.qryConfigLOGOMARCA.Value;
 Fast.TipoDANFE  := tiNFCe;
 Fast.PathPDF  := Dados.qryConfigPATHNFE.AsString;
 Fast.FastFile := ExtractFilePath(Application.ExeName) + '\Relatorio\'+ Dados.LerReg_ModeloDanfe;
 Fast.Logo     := Dados.qryConfigLOGOMARCA.AsString;
 Fast.MargemInferior:=0.0;
 Fast.MargemSuperior:=0.0;
 Fast.MargemEsquerda:=0.0;
 Fast.MargemDireita:=0.0;
 Fast.Sistema := Dados.qryParametroEMPRESA.Value + ' | ' +
 Dados.qryParametroFONE1.Value + ' ' + Dados.qryParametroFONE2.Value;
 Fast.Site := Dados.qryParametroSITE.Value;
end;
