...
uses
  ...
  MemDS, DBAccess, Uni, UniProvider, MySQLUniProvider, Winapi.Windows, FireDAC.VCLUI.Error;
...
begin
  try
    ...
    begin
      ...
      if not DadosWeb.qryEmpresa.fieldbyname('ie').IsNull then
        dados.qryEmpresaMENSALIDADE.Value := DadosWeb.qryEmpresa.fieldbyname('ie').AsString;

      if not DadosWeb.qryEmpresa.fieldbyname('cnpj_representante').IsNull then
        dados.qryEmpresaCNPJ_REPRESENTANTE.AsString := DadosWeb.qryEmpresa.fieldbyname('cnpj_representante').AsString;
      ...
      if qryTerminais.Locate('MAC', Dados.GetMacAddress, []) then
        begin
          if qryTerminais.FieldByName('NOME_COMPUTADOR').Value <>  Dados.Getcomputer then
...
begin
  ...
  try
    ...
    begin
      ...
      DadosWeb.qryEmpresa.fieldbyname('validade_licenca').Value := datetostr(Date + 10);
    end
    else
      begin
    ...
    DadosWeb.qryEmpresa.fieldbyname('vData').Value :=
      datetostr(vData);
      if DadosWeb.qryEmpresa.fieldbyname('ie').Value = '' then
      begin
      DadosWeb.qryEmpresa.fieldbyname('ie').Value := 100;
      end
      else
      begin
      DadosWeb.qryEmpresa.fieldbyname('ie').Value := dados.qryEmpresaMENSALIDADE.Value;
      end;
    DadosWeb.qryEmpresa.fieldbyname('numero').Value := dados.qryEmpresanumero.Value;
    ...
    if qryTerminais.RecordCount <= nTerminais then
      begin
        if not (qryTerminais.Locate('MAC', Dados.GetMacAddress, [])) then
          begin
            qryTerminais.Insert;
...
procedure TDadosWeb.CadastroTerminais;
begin
  qryTerminais.Close;
...
//        if not (qryTerminais.Locate('MAC', dados.crypt('C', Dados.GetMacAddress), [])) then
        if not (qryTerminais.Locate('MAC', (Dados.GetMacAddress), [])) then
          ...
          begin
            qryTerminais.Edit;
  //          qryTerminais.FieldByName('NOME_COMPUTADOR').Value := dados.crypt('C', Dados.Getcomputer);
            qryTerminais.FieldByName('NOME_COMPUTADOR').Value := Dados.Getcomputer;
            qryTerminais.Post;
          end;
end;
