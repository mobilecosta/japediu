...
type
  TfrmHistoricoVendaProduto_VDD = class(TForm)
    ...
    Comissao: TFDMemTable;
    ComissaoCOMISSAO_P: TFloatField;
    ComissaoCOMISSAO_V: TFloatField;
    qryHistoricoT: TFDQuery;
    qryHistoricoTVENDEDOR: TStringField;
    qryHistoricoTFORMA: TStringField;
    qryHistoricoTVALOR_PAGO: TFMTBCDField;
    qryHistoricoTCOMISSAO_P: TFloatField;
    qryHistoricoTCOMISSAO_V: TFloatField;
    frxDBHistoricoT: TfrxDBDataset;
...
procedure TfrmHistoricoVendaProduto_VDD.Totais;
var
  Data1, Data2: TDate;
...
begin
  ...
  qryHistoricoT.close;
  qryHistoricoT.Params[0].Value := Data1;
  qryHistoricoT.Params[1].Value := Data2;
  qryHistoricoT.Params[2].Value := '%';

  if cbVendedor.KeyValue <> 0 then
    qryHistoricoT.Params[2].Value := cbVendedor.KeyValue;
  ...
  frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
    '\Relatorio\RelHistoricoVendaProdutoVddT.fr3');
  frxReport.ShowReport;
...
begin
  ...
  if cbTipo.Text = 'Resumido' then
    Resumo
  else if cbTipo.Text = 'Detalhado'  then
    Detalhado
    else
     Totais;
...
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Histórico de Vendas de Produtos p/ Vendedor';
