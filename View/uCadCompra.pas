          ...
          begin
            ...
            begin
              ...
              begin
                ...
                dados.qryExecute.SQL.Text :=
                  'update COMPRA_ITENS set VL_ST=:VALOR, ALIQ_ST=:ALIQ, VL_UNITARIO=:VL_UNITARIO, VL_ITEM=:VL_ITEM  WHERE FK_COMPRA=:FK AND BASE_ST=:BASE';
...
procedure TfrmCadCompra.ChamaLote;
var
  qryPesquisa: TFDQuery;
  eLote: Boolean;
begin
  try
    ...
    qryPesquisa.SQL.Clear;
    qryPesquisa.SQL.Text :=
      ' select USA_LOTE from produto where codigo=:codigo and USA_LOTE=''S''';
    qryPesquisa.Params[0].Value := qryItensCompraFK_PRODUTO.AsInteger;
    qryPesquisa.Open;

    eLote := qryPesquisa.FieldByName('USA_LOTE').AsString = 'S';
    ...
    begin
      ...
      qryPesquisa.SQL.Clear;
      qryPesquisa.SQL.Text :=
        'DELETE from COMPRA_LOTE where FK_ITENS_COMPRA=:ID';
      qryPesquisa.Params[0].Value := qryItensCompraID.AsInteger;
      qryPesquisa.ExecSQL;
      try
        FrmLoteCo := TFrmLoteCo.Create(Application);
        FrmLoteCo.GeraLote(qryItensCompraFK_PRODUTO.AsInteger,
          qryCompraID.AsInteger, qryItensCompraID.AsInteger);
        FrmLoteCo.AbreLote(qryItensCompraID.AsInteger);
        FrmLoteCo.edtQTD.Text := qryItensCompraQTD.AsString;
        FrmLoteCo.showmodal;
      finally
        FrmLoteCo.Free;
      end;
...
begin
  ...
  dados.qryupdate.ParamByName('FK_PRODUTO').Value :=
    dados.buscacodigoProduto('select * from produto where codbarra=:cod or referencia =:ref',
    vPesquisa);
  ...
  dados.qryupdate.ParamByName('VL_UNITARIO').Value := qryItensCompraVIRTUAL_PRECO.AsFloat;
  dados.qryupdate.ParamByName('PR_VENDA').Value := qryItensCompraVIRTUAL_PRECO.AsFloat;
