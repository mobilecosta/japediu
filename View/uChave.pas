...
uses
  ...
  cxClasses, dxSkinsForm, cxLabel, cxTextEdit, cxMaskEdit, cxProgressBar, dxSkinsCore,
...
uses Udados, uDadosWeb, uAPITelegramMensagem;
...
//        LogTelegramBot('Realizado Liberação do Sistema', ' Chave Executada com sucesso', ' Retaguarda');
...
begin
  ...
  begin
    ...
    begin
      ...
      dados.Conexao.Commit;
      LogTelegramBot('Envio de licença - ',' Maquina realizada: '+ dados.Getcomputer ,' - Retaguarda') ;
      close;
    end;
  end
  else
    ShowMessage('Chave Incorreta.');
    LogTelegramBot('ERROR | licença - ','Erro ao registrar divergencia no serial', ' - Retaguarda') ;
end;
