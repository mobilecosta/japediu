...
uses
  ...
  ACBrDANFCeFortesFrA4, frxExportXLS, System.Actions, Vcl.ActnList, Vcl.Menus;
...
type
  TfrmNFCe = class(TForm)
    ...
    qryPesPVSELECAO: TIntegerField;
    StringField7: TStringField;
    dsPesPV: TDataSource;
    PopupMenu: TPopupMenu;
    miRecaConti: TMenuItem;
    miEditarVenda: TMenuItem;
    tmrAutoNFCe: TTimer;
    pmRelatorio: TPopupMenu;
    RelatrioNFCe1: TMenuItem;
    RelatrioItensNFCe1: TMenuItem;
    qryFormaPagamento: TFDQuery;
    qryFormaPagamentoID_FORMA: TIntegerField;
    qryFormaPagamentoDESCRICAO: TStringField;
    qryFormaPagamentoTOTAL: TFMTBCDField;
    qryRelatorioItens: TFDQuery;
    qryRelatorioItensID_PRODUTO: TIntegerField;
    qryRelatorioItensPRODUTO: TStringField;
    qryRelatorioItensTOTAL: TFMTBCDField;
    qryRelatorioItensSITUACAO: TStringField;
    qryRelatorioItensNUMERO: TIntegerField;
    qryRelatorioItensDATA_EMISSAO: TDateField;
    qryRelatorioItensVIRTUAL_SITUACAO: TStringField;
    TabSheet4: TTabSheet;
    dsFPG: TDataSource;
    qryFPGG: TFDQuery;
    ...
    procedure miRecaContiClick(Sender: TObject);
    procedure miEditarVendaClick(Sender: TObject);
    procedure tmrAutoNFCeTimer(Sender: TObject);
    procedure RelatrioNFCe1Click(Sender: TObject);
    procedure RelatrioItensNFCe1Click(Sender: TObject);
  private
    ...
    procedure Botoes;
    procedure CorrigeDivergencia;
    function ExisteProdutosForaLinha(Venda: Integer): Boolean;

    procedure TTransmitirAuto;
    procedure sTransmitidaAuto;
    procedure UpdateErroNFCe(xErro: string; codigo: Integer);
    procedure sCanceladaAuto;
    procedure sDuplicidadeAuto;
    procedure sDenegadoAuto;
    procedure sInutilizadaAuto;

    procedure FiltrarRelatorioItens;

    procedure ModuloWhatsApp;
    { Private declarations }
  public
    idx: Integer;
    vOrdem, vSql, vSqlFPG, vSqlTrib: String;
    FTodos: Boolean;
    procedure ADDPedido(vNum: String);
    procedure EnvioNFCeAuto;
    { Public declarations }
  end;
...
uses Udados, ufrmStatus, uEmail, uCadProduto, uDadosWeb, uDmNFe, uDMEstoque,
  uRecarregarNFCe, uVendaVincNFCe, uNFCeAutoEnvio;

procedure TfrmNFCe.Botoes;
begin
  btnCancelar.Visible := Dados.qryPermissoesBotaoEDITAR.Value = 'S';
  bbInutilizar.Visible := Dados.qryPermissoesBotaoEDITAR.Value = 'S';
  btnTransmitir.Visible := Dados.qryPermissoesBotaoEDITAR.Value = 'S';
  btnRecuperar.Visible := Dados.qryPermissoesBotaoEDITAR.Value = 'S';
  btnLote.Visible := Dados.qryPermissoesBotaoEDITAR.Value = 'S';
end;
...
procedure TfrmNFCe.EnvioNFCeAuto;
begin
  ...
  begin
    try
      ...
      frmNFCeAutoEnvio := TfrmNFCeAutoEnvio.Create(Application);
      frmNFCeAutoEnvio.Left := (frmNFCe.ClientWidth div 2) -
        (frmNFCeAutoEnvio.Width div 2);
      frmNFCeAutoEnvio.Top := (frmNFCe.ClientHeight div 2) -
        (frmNFCeAutoEnvio.Height div 2);
      frmNFCeAutoEnvio.Top := frmNFCeAutoEnvio.Top -
        (frmNFCeAutoEnvio.Height + 10);
      frmNFCeAutoEnvio.Show;
...
function TfrmNFCe.ExisteProdutosForaLinha(Venda: Integer): Boolean;
var
  qry: TFDQuery;
begin
  try
...
procedure TfrmNFCe.ADDPedido(vNum: String);
begin
  qryPedido.Close;
  qryPedido.Params[0].Value := StrToIntDef(vNum, 0);
  qryPedido.Open;
  ...
  if qryDocumento.Locate('CODIGO', vNum) then
    exit;
...
procedure TfrmNFCe.RelatrioItensNFCe1Click(Sender: TObject);
begin
  FiltrarRelatorioItens;
  try
    btnRelatorio.Enabled := false;
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\ListadeNFCeItens.fr3');
...
procedure TfrmNFCe.RelatrioNFCe1Click(Sender: TObject);
begin
  CorrigeDivergencia;
  FiltraRelatorioR('RELATORIO');
  FiltraRelatorioD('RELATORIO');
  FiltraImposto('RELATORIO');
  FiltraFormaPagamento('RELATORIO');
    ...
    begin
      ...
      begin
        ...
        QRYNFCE_DVALOR_ICMS.AsFloat :=
          ...
          qryItensPedidoTOTAL.AsFloat) / 100, -2);
      end;
...
var
  dia, mes, ano: Word;
begin
  if not bbInutilizar.Visible then
    exit;
...
var
  ...
  iQtdFPG: Integer;
  rValorFPG, rValorFPGAux: Extended;
begin
  try
    vMsg := '';
    FAliqDeson := 0;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          with Imposto do
          begin
            vTotTrib := QRYNFCE_DTRIB_EST.AsFloat + QRYNFCE_DTRIB_FED.AsFloat;
            // QRYNFCE_DTRIB_MUN.AsFloat + QRYNFCE_DTRIB_IMP.AsFloat +
            ...
            begin
              ...
              begin // empresas que não são do simples nacional
                ...
                ICMS.pMVAST := 0; // QRYNFCE_DALIQ_ICMS.Value;;
                ICMS.pRedBCST := 0;
                ICMS.vBCST := 0;
                ICMS.pICMSST := 0;
                ICMS.vICMSST := 0;
      ...
      Total.ICMSTot.vTotTrib := qryNFCE_MTRIB_EST.AsFloat +
        qryNFCE_MTRIB_FED.AsFloat;
      // qryNFCE_MTRIB_MUN.AsFloat +   qryNFCE_MTRIB_IMP.AsFloat +
      ...
      begin
        if qryNFCE_MTOTAL.AsFloat = qryVendaTOTAL.AsFloat then
        begin
          qryTotalFPG.Close;
      ...
      if (qryNFCE_MTROCO.AsFloat <> rValorFPGAux - Total.ICMSTot.vNF) then
        pag.vTroco := rValorFPGAux - Total.ICMSTot.vNF
      else
...
{$REGION 'Ajustando forma de pagamento para produtos fora de linha'}
      { if ExisteProdutosForaLinha(qryNFCE_MFK_VENDA.AsInteger) then
        begin
        //Refazer forma de pagamentos
        pag.Clear;
        ...
        rValorFPG := RoundTo(qryNFCE_MTOTAL.AsFloat / iQtdFPG, -2);
        rValorFPGAux  := RoundTo((iQtdFPG * rValorFPG) - qryNFCE_MTOTAL.AsFloat, -2);
        qryTotalFPG.First;
        ...
        begin
        tPag := fpDinheiro;
        if qryTotalFPG.Eof then
        vPag := rValorFPG - rValorFPGAux
        else
        vPag := rValorFPG;
        end;
        end;
...
procedure TfrmNFCe.TTransmitirAuto;
begin
  Application.ProcessMessages;
  ...
  // Verificar se está Off - WS
  GerarNFCe(Dados.qryNFCE_MNUMERO.AsString); // passo 4 gerar xml

  // ACBrNFe.NotasFiscais.Items[0].GravarXML(ExtractFilePath(Application.ExeName)+
  // Dados.qryNFCE_MCHAVE.AsString+'.xml');
      ...
      case dmnfe.ACBrNFe.webservices.Enviar.cStat of
        ...
        begin
          case dmnfe.ACBrNFe.webservices.Retorno.cStat of
            ...
            begin
              UpdateErroNFCe('Retorno:' + dmnfe.ACBrNFe.webservices.Retorno.
                cStat.ToString + ' - ' + dmnfe.ACBrNFe.webservices.Retorno.
                xMotivo + sLineBreak + 'Erro:' + e.Message,
                Dados.qryNFCE_MCODIGO.AsInteger);
              exit;
            end;
          end;
        end;
...
procedure TfrmNFCe.UpdateErroNFCe(xErro: string; codigo: Integer);
var
  qry: TFDQuery;
begin
  try
    ...
    qry.Close;
    qry.SQL.Clear;
    qry.SQL.Text := 'update nfce_master nm set ' + ' nm.xerro=' +
      QuotedStr(xErro) + ' where nm.codigo=' + IntToStr(codigo);
    qry.ExecSQL;
  finally
    qry.Free;
  end;
...
begin
  ...
  begin
    ...
    dmEstoque.AtualizaEstoqueFiscal('NFCE', qryVendaCODIGO.Value, 'S',
      Dados.qryEmpresaEXIBE_ESTOQUE_FISCAL.Value);
    UpdateErroNFCe('NFCe transmitido com sucesso', qryVendaCODIGO.AsInteger);
  end;
end;

procedure TfrmNFCe.sTransmitidaAuto;
begin
  if not(qryVenda.State in dsEditModes) then
    qryVenda.Edit;
  qryVendaPROTOCOLO.Value := dmnfe.ACBrNFe.NotasFiscais.Items[0]
...
begin
  ...
  UpdateErroNFCe('Retorno:' + dmnfe.ACBrNFe.webservices.Enviar.cStat.ToString +
    ' - ' + dmnfe.ACBrNFe.webservices.Enviar.xMotivo, qryVendaCODIGO.AsInteger);
end;

procedure TfrmNFCe.sCanceladaAuto;
begin
  if not(qryVenda.State in dsEditModes) then
    qryVenda.Edit;
  qryVendaCHAVE.Value := copy(dmnfe.ACBrNFe.NotasFiscais.Items[0]
...
procedure TfrmNFCe.sDuplicidadeAuto;
begin
  if not(qryVenda.State in dsEditModes) then
    qryVenda.Edit;
  qryVendaFLAG.Value := 'N';
...
procedure TfrmNFCe.sInutilizadaAuto;
begin
  if not(qryVenda.State in dsEditModes) then
    qryVenda.Edit;
  qryVendaCHAVE.Value := copy(dmnfe.ACBrNFe.NotasFiscais.Items[0]
...
procedure TfrmNFCe.sDenegadoAuto;
begin
  if not(qryVenda.State in dsEditModes) then
    qryVenda.Edit;
  qryVendaCHAVE.Value := copy(dmnfe.ACBrNFe.NotasFiscais.Items[0]
...
procedure TfrmNFCe.AtualizaNFCe(aCodigo: Integer; aXML: String);
var
  N, i, j: Integer;
begin
  if (Trim(aXML) = '') then
  ...
  dmnfe.ACBrNFe.NotasFiscais.Clear;
  dmnfe.ACBrNFe.NotasFiscais.LoadFromString(aXML);
  ...
  try
    ...
    qryNFCE_M.Close;
    qryNFCE_M.Params[0].AsInteger := aCodigo;
    qryNFCE_M.Open;

    QRYNFCE_D.Close;
    QRYNFCE_D.Params[0].AsInteger := aCodigo;
    QRYNFCE_D.Open;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            QRYNFCE_D.Insert;
            QRYNFCE_DFKVENDA.Value := aCodigo;
            QRYNFCE_DCODIGO.Value := Dados.Numerador('NFCE_DETALHE', 'CODIGO',
              'N', '', '');
...
begin
  ...
  begin
    ...
    case TabSet1.TabIndex of
      0:
        filtro := ' NM.SITUACAO = ''T'' AND ';
      1:
        filtro := ' NM.SITUACAO = ''D'' AND';
      2:
        filtro := ' NM.SITUACAO = ''I'' AND';
      3:
        filtro := ' NM.SITUACAO = ''G'' AND';
      4:
        filtro := ' NM.SITUACAO = ''O'' AND';
      5:
        filtro := ' NM.SITUACAO = ''C'' AND';
      6:
        filtro := ' NM.SITUACAO = ''X'' AND';
    end;
  end
  else
    filtro := ' NM.SITUACAO IN (''T'',''I'',''C'') AND';
...
procedure TfrmNFCe.FiltrarRelatorioItens;
var
  filtro: string;
begin
  filtro := '';
  ...
  qryRelatorioItens.SQL.Text := vsql4;
  qryRelatorioItens.SQL.Text := stringreplace(qryRelatorioItens.SQL.Text,
    '/*pSituacao*/', filtro, []);
  qryRelatorioItens.Params[0].AsDate := strtodate(maskInicio.Text);
  qryRelatorioItens.Params[1].AsDate := strtodate(maskFim.Text);
  qryRelatorioItens.Params[2].Value := Dados.qryEmpresaCODIGO.Value;
...
begin
  ...
  if not btnTransmitir.Visible then
    exit;
...
procedure TfrmNFCe.CorrigeDivergencia;
var
  qryDiferente: TFDQuery;
begin
  try
    qryDiferente := TFDQuery.Create(self);
    qryDiferente.Connection := Dados.Conexao;

    qryDiferente.Close;
    qryDiferente.SQL.Clear;
    qryDiferente.SQL.Add('select');
    qryDiferente.SQL.Add('NM.codigo,');
    qryDiferente.SQL.Add('nm.xml,');
    qryDiferente.SQL.Add('NM.total,');
    qryDiferente.SQL.Add('sum(nd.TOTAL)as total');
    qryDiferente.SQL.Add('FROM NFCE_MASTER NM');
    qryDiferente.SQL.Add('left join nfce_detalhe nd on nd.fkvenda=nm.codigo');
    qryDiferente.SQL.Add('WHERE');
    qryDiferente.SQL.Add('NM.SITUACAO<>''I'' AND');
    qryDiferente.SQL.Add('NM.data_emissao between :DATA1 AND :DATA2 AND');
    qryDiferente.SQL.Add('NM.FKEMPRESA=:EMP');
    qryDiferente.SQL.Add('group by 1,2,3');
    qryDiferente.SQL.Add('having sum(nd.TOTAL)<> nm.total');
    qryDiferente.Params[0].AsDate := strtodate(maskInicio.Text);
    qryDiferente.Params[1].AsDate := strtodate(maskFim.Text);
    qryDiferente.Params[2].Value := Dados.qryEmpresaCODIGO.Value;
    qryDiferente.Open;

    if not qryDiferente.IsEmpty then
    begin
      AtualizaNFCe(qryDiferente.FieldByName('codigo').AsInteger,
        qryDiferente.FieldByName('xml').AsString);
      qryDiferente.Next;
    end;
...
begin
  AtualizaNFCe(qryVendaCODIGO.Value, qryVendaXML.AsString);
end;

procedure TfrmNFCe.Button3Click(Sender: TObject);
begin
  try
    qryPesPv.First;
    while not qryPesPv.Eof do
    begin
      if qryPesPVSELECAO.Value = 1 then
      begin
        ADDPedido(IntToStr(qrypesPvCodigo.Value));
      end;
      qryPesPv.Next;
    end;
  finally
...
procedure TfrmNFCe.Button4Click(Sender: TObject);
begin
  PagUtilidade.Visible := false;
end;
...
procedure TfrmNFCe.BitBtn3Click(Sender: TObject);
begin
  qryPesPv.Close;
  qryPesPv.ParamByName('data1').AsDate := strtodate(MaskEdit1.Text);
  qryPesPv.ParamByName('data2').AsDate := strtodate(MaskEdit2.Text);
  if Dados.qryEmpresaPESQUISA_POR_PARTE.Value = 'S' then
    qryPesPv.ParamByName('nome').Value := '%' + edtPessoa.Text + '%'
  else
    qryPesPv.ParamByName('nome').Value := edtPessoa.Text + '%';
  qryPesPv.Open;
  qryPesPv.Locate('codigo', StrToIntDef(edtNumero.Text, 0), []);
end;
...
begin
  ...
  DBGrid1.Columns[7].Width := round(Screen.Width * 0.08);
  DBGrid1.Columns[8].Width := round(Screen.Width * 0.08);
  DBGrid1.Columns[9].Width := round(Screen.Width * 0.08);
  DBGrid1.Columns[10].Width := round(Screen.Width * 0.08);
end;
...
procedure TfrmNFCe.tmrAutoNFCeTimer(Sender: TObject);
begin
  tmrAutoNFCe.Enabled := false;
  EnvioNFCeAuto;
end;
...
begin
  ...
  DBGrid1.Columns[10].Title.caption := 'Nº Pedido';
...
procedure TfrmNFCe.DBGrid4DblClick(Sender: TObject);
begin
...
procedure TfrmNFCe.DBGrid4DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
  Check: Integer;
  R: TRect;
...
begin
  ...
  qryFPGG.Close;
  qryFPGG.Params[0].Value := qryVendaFK_VENDA.Value;
  qryFPGG.Open;
end;
...
var
  dia, mes, ano: Word;
begin
  caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - NFC-e';
  vOrdem := ' ASC';
...
procedure TfrmNFCe.ModuloWhatsApp;
begin
...
begin
  ...
  if vsql4 = '' then
    vsql4 := qryRelatorioItens.SQL.Text;
...
procedure TfrmNFCe.FiltraFormaPagamento(tipo: string);
var
  filtro: string;
begin
  filtro := '';
  ...
  if vSqlFPG = '' then
    vSqlFPG := qryFormaPagamento.SQL.Text;
  ...
  qryFormaPagamento.SQL.Text := vSqlFPG;
  qryFormaPagamento.SQL.Text := stringreplace(qryFormaPagamento.SQL.Text,
    '/*where*/', filtro, [rfReplaceAll]);
  qryFormaPagamento.Params[0].AsDate := strtodate(maskInicio.Text);
  qryFormaPagamento.Params[1].AsDate := strtodate(maskFim.Text);
  qryFormaPagamento.Params[2].Value := Dados.qryEmpresaCODIGO.Value;
...
begin
  ...
  if vSqlTrib = '' then
    vSqlTrib := qryTributacao.SQL.Text;
  ...
  qryTributacao.Close;
  qryTributacao.SQL.Text := vSqlTrib;
    ...
    begin
      ...
      case idx of
        ...
        10:
          begin
            if (Trim(edtLoc.Text) <> '') then
              filtro := filtro + ' and vm.FK_VENDA like ' +
                QuotedStr(parte + edtLoc.Text + '%');
          end;
      ...
      case idx of
        ...
        10:
          ordem := ' order by vm.serie,vm.FK_VENDA ' + vOrdem;
      end;
...
procedure TfrmNFCe.miEditarVendaClick(Sender: TObject);
var
  InputString, Senha: string;
begin
  PostMessage(Handle, InputBoxMessage, 0, 0);
  Senha := FormatDateTime('ddyyyy', DATE);
  InputString := InputBox('Suporte Técnico', 'Digite sua senha', '');
  if InputString = Senha then
  begin
    try
      frmVendaVincNFCe := TfrmVendaVincNFCe.Create(Application);
      frmVendaVincNFCe.iCodigoVenda := qryVendaFK_VENDA.AsInteger;
      frmVendaVincNFCe.ShowModal;
    finally
      frmVendaVincNFCe.Release;
    end;
  end
  else
...
procedure TfrmNFCe.miRecaContiClick(Sender: TObject);
var
  sErro: string;
begin
  try
    if Application.messagebox('Deseja Recarregar Todas?', 'Confirmação',
      mb_yesno + mb_iconquestion) = idyes then
    begin
      qryVenda.First;
      while not qryVenda.Eof do
      begin
        FTodos := true;
        AtualizaNFCeVenda(qryVendaFK_VENDA.AsInteger,
          qryVendaCODIGO.AsInteger, sErro);
...
procedure TfrmNFCe.ChkVendaClick(Sender: TObject);
begin
  try
    qryPesPv.DisableControls;
    qryPesPv.First;
    while not qryPesPv.Eof do
    begin
      qryPesPv.Edit;
      qryPesPVSELECAO.Value := 1;
      qryPesPv.Post;
      qryPesPv.Next;
    end;
  finally
    qryPesPv.EnableControls;
  end;
end;
