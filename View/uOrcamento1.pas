...
begin
  ...
  try
    ...
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelOrcamentoFoto.fr3');
...
begin
  if Dados.qryOrcamentoSITUACAO.Value = 'C' then
  begin
    ShowMessage('Orçamento Cancelado não pode ser Impresso!');
  end
  else
  begin
...
begin
  try
    ...
    begin
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelOrcamento80mm.fr3');
      frxReport.PrepareReport;
    ...
    begin
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelOrcamento80mmSemPreço.fr3');
      frxReport.PrepareReport;
    //  frxReport.PrintOptions.Printer := Dados.qryTerminalPORTA.AsString;
    //  frxReport.PrintOptions.ShowDialog := false;
      frxReport.ShowReport;
...
{ procedure TfrmOrcamento.imprimepedido58mm;
  var
  sTexto, vEndereco: String;
  SL: TStringList;
  begin
  ...
  SL.Add('<N>' + copy(Dados.qryEmpresaFANTASIA.AsString, 1, 40) + '</n>');
  SL.Add(Dados.qryEmpresaENDERECO.AsString + ',' +
  Dados.qryEmpresaNUMERO.Value);
  SL.Add(Trim(Dados.qryEmpresaBAIRRO.AsString) + ' - ' +
  Trim(Dados.qryEmpresaCIDADE.AsString) + '-' +
  ...
  Dados.qryOrcamentoNUMERO.AsString;
  // SL.Add('</linha_simples>');
  sTexto := '*** ORÇAMENTO No. ' + FormatFloat('0000',
  Dados.qryOrcamentoCODIGO.AsFloat) + ' ***</CE></n>';
  SL.Add(sTexto);
  // SL.Add('</linha_simples>');
  sTexto := copy(Dados.qryOrcamentoCLIENTE.AsString, 1, 40);
  SL.Add('Cliente...: ' + sTexto);
  sTexto := copy(vEndereco, 1, 40);
  SL.Add('Endereco..: ' + sTexto);
  sTexto := copy(Dados.qryOrcamentoBAIRRO.AsString, 1, 40);
  SL.Add('Bairro....: ' + sTexto);
  sTexto := copy(Dados.qryOrcamentoTELEFONE.AsString, 1, 40);
  SL.Add('Telefone..: ' + sTexto);
  sTexto := copy(Dados.qryOrcamentoDATA.AsString, 1, 40);
  SL.Add('Data:' + sTexto + ' Hora:' + timetostr(time));
  SL.Add('Vendedor:' + Dados.qryOrcamentoVENDEDOR.Value);
  // SL.Add('</linha_simples>');
  // SL.Add('<C>COD PRODUTO                       QTD    VALOR     TOTAL</C>');
  SL.Add('<C>COD   PRODUTO</C>');
  SL.Add('<C>QTD   VALOR     TOTAL</C>');
  // SL.Add('</linha_simples>');
  Dados.qryItensO.First;
  While not Dados.qryItensO.Eof do
  begin
  sTexto := Dados.Alinha('', 1, false) +
  Dados.Alinha(Dados.qryItensO.FieldByName('FK_PRODUTO').AsString, 06,
  false) + Dados.Alinha('', 1, false) +
  Dados.Alinha(copy(Dados.qryItensOVIRTUAL_PRODUTO.AsString, 1, 22) + ' '
  + copy(Dados.qryItensOgrade.AsString, 1, 8), 22, false) +
  Dados.Alinha('', 1, false) + Dados.Alinha(Dados.qryItensOQTD.AsString,
  ...
  SL.Add(Dados.qryOrcamentoOBS.Value);
  // SL.Add('</linha_dupla>');
  SL.Add('Relatorio emitido em ' + DateTimeToStr(now));
  // SL.Add('</corte>');
...
begin
  try
    ...
    begin
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelOrcamento58mm.fr3');
      frxReport.PrepareReport;
      frxReport.PrintOptions.Printer := Dados.qryTerminalPORTA.AsString;
      frxReport.PrintOptions.ShowDialog := false;
      frxReport.Print;
    ...
    begin
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelOrcamento58mmSemPreço.fr3');
      frxReport.PrepareReport;
      frxReport.PrintOptions.Printer := Dados.qryTerminalPORTA.AsString;
      frxReport.PrintOptions.ShowDialog := false;
      frxReport.Print;
...
begin
  ...
  if (Dados.qryOrcamentoSITUACAO.Value = 'A') Then
    DBGrid1.Canvas.Font.Color := clBlack;

  if (Dados.qryOrcamentoSITUACAO.Value = 'C') Then
    DBGrid1.Canvas.Font.Color := clRed;

  if (Dados.qryOrcamentoSITUACAO.Value = 'I') Then
...
procedure TfrmOrcamento.Botoes;
begin
...
begin
  Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Orçamentos';
  maskInicio.Text := DateToStr(StartOfTheMonth(Now));
...
procedure TfrmOrcamento.ModuloWhatsApp;
begin
...
begin
  ...
  case TabSet1.TabIndex of
    ...
    4:
      filtro1 := ' and orc.situacao=''I'''; // Importado
  end;
