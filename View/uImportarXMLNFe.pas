...
type
  TfrmImportaXMLNFe = class(TForm)
    ...
    qryItensINFO_ADICIONAIS: TStringField;
    qryItensCEST: TStringField;
    qryItensPRECO: TFMTBCDField;
    qryItensNPEDIDO: TIntegerField;
...
function TfrmImportaXMLNFe.TemAtributo(Attr, Val: Integer): Boolean;
begin
  Result := Attr and Val = Val;
end;

procedure TfrmImportaXMLNFe.ListarArquivos(Diretorio: string; Sub: Boolean);
var
  F: TSearchRec;
  Ret: Integer;
  TempNome: string;
  ...
  try
    ...
    begin
      try
        ...
        if ACBrNFe.NotasFiscais.Items[0].NFe.Ide.modelo = 55 then // se for nfce
        begin
          ...
          begin
            qryVenda.Insert;
            qryVendaCODIGO.Value := dados.Numerador('NFE_MASTER', 'CODIGO',
              'N', '', '');
          end
          else
            qryVenda.edit;
          ...
          qryVendaFRETE.Value := ACBrNFe.NotasFiscais.Items[0]
            .NFe.Total.ICMSTot.vFrete;
          qryVendaSEGURO.Value := ACBrNFe.NotasFiscais.Items[0]
            .NFe.Total.ICMSTot.vSeg;
          qryVendaOUTROS.Value := ACBrNFe.NotasFiscais.Items[0]
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  qryItensFKNFE.Value := qryVendaCODIGO.Value;
                  qryItensCODIGO.Value := dados.Numerador('NFE_DETALHE',
                    'CODIGO', 'N', '', '');
                  ...
                  qryItensPRECO.Value := prod.vProd;
                  qryItensCOD_BARRA.Value := prod.cEAN;
                  qryItensDESCONTO.Value := prod.vDesc;
                  qryItensFRETE.Value := prod.vFrete;
                  qryItensDESPESAS.Value := prod.vOutro;
                  qryItensSEGURO.Value := prod.vSeg;
                  qryItensBLOQUEADO.Value := 'S';
                  qryItensGERA_ES.Value := 'N';
                  qryItensCST.Value := OrigToStr(Imposto.ICMS.orig) +
                    CSTICMSToStr(Imposto.ICMS.CST);
                  qryItensCSOSN.Value := CSOSNIcmsToStr(Imposto.ICMS.CSOSN);
...
procedure TfrmImportaXMLNFe.FormCreate(Sender: TObject);
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Importar XML de NFe';
end;

procedure TfrmImportaXMLNFe.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = vk_f3 then
    btnImportar.Click;
