...
uses
  ...
  DBGridEh, DBCtrlsEh,
  DBLookupEh, frxExportXLS, Vcl.Imaging.pngimage, JvComponentBase, JvEnterTab;
...
begin
  if not btnNovo.Visible = false then
    exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (Length(FFone) = 11) or ((Length(FFone) = 10)) then
      begin
        dados.InsereMensagemZap('', FFone, Parcelas +sLineBreak +sLineBreak ,
        dados.qryEmpresaMENSAGEM_COBRANCA.Value,
          qryInadimplentes.FieldByName('FANTASIA').Value,
          dados.qryEmpresa.FieldByName('FANTASIA').Value, 'PARCELAS EM ATRASO');
...
begin
  ...
  dbGrid1.Columns[3].Title.Caption := 'Modalidade';
  dbGrid1.Columns[4].Title.Caption := 'Obs';
