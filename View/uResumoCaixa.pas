...
begin
  if FResumo.Dinheiro = 0 then
    if edtValor.Value <> 0 then
     Showmessage('Atenção! Total em dinheiro no caixa é zero!!');

  if FResumo.Dinheiro <> 0 then
    if edtValor.Value = 0 then
      Showmessage('Atenção! Informe o total em dinheiro no caixa!!');
...
procedure TfrmResumoCaixa.FormCreate(Sender: TObject);
begin
  Label11.Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Remetente';
end;
...
//    ShowMessage(frmResumoCaixa.qryConsulta.SQL.Text);
