...
begin
  try
    ...
    if MessageDlg('Deseja imprimir em duas vias?', mtConfirmation, mbYesNo,0) = mrYes then
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelRecibo.fr3')
    else
     frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelRecibo_1via.fr3');
...
begin
  Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Recibo';
  vOrdem := 'ASC';
