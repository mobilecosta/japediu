...
begin
  ...
  begin
    ...
    if (DBComboBoxEh1.Text = 'DINHEIRO') or (DBComboBoxEh1.Text = 'PIX') then
    begin
...
begin
  ...
  if DBComboBoxEh1.Text = 'PAGAMENTO INSTANTÂNEO (PIX)' then
  begin
...
procedure TfrmFPG.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  close;
end;
...
procedure TfrmFPG.SpeedButton1Click(Sender: TObject);
begin
  Open.Execute;
  if trim(Open.FileName) <> '' then
    edtImagem.Text := Open.FileName;
end;
