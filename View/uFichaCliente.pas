...
begin
  if not btnExcluir.Visible then
    exit;
...
procedure TfrmFichaCliente.Botoes;
begin
  btnNovo.Visible := dados.qryPermissoesBotaoINCLUIR.Value = 'S';
  btnAlterar.Visible := dados.qryPermissoesBotaoEDITAR.Value = 'S';
  btnReceber.Visible := dados.qryPermissoesBotaoEDITAR.Value = 'S';
  btnExcluir.Visible := dados.qryPermissoesBotaoEXCLUIR.Value = 'S';
end;

procedure TfrmFichaCliente.FormCreate(Sender: TObject);
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Ficha de Clientes';
