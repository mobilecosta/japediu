...
begin
  ...
  Dados.qryPrincipioCODIGO.Value := Dados.Numerador('PRINCIPIO_ATIVO', 'CODIGO',
    'N', '', '');
  Panel1.Enabled := true;
  DBEdit2.SetFocus;
...
begin
  if Application.messageBox
    ('Tem Certeza de que deseja sair do cadastro de Princípio?', 'Confirmação',
    mb_YesNo) = mrYes then
    close;
end;
...
begin
  Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Cadastro de Princípios Ativo';
  vOrdem := 'asc';
end;
