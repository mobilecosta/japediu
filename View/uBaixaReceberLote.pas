...
uses
  ...
  frxDBSet, dxSkinsCore, dxSkinDevExpressDarkStyle, dxSkinOffice2019Colorful,
...
implementation
{$R *.dfm}
uses Udados, uCadReceber;
procedure TfrmBaixaReceberLote.cxGravarClick(Sender: TObject);
begin
...
procedure TfrmBaixaReceberLote.cxSairClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        qryCR.Post;
            if (dados.truncar(qryBaixaLoteTOTAL_FINAL.asFloat, 3)) >= 0.02 then
             begin
                 if qryCRSITUACAO.Value = 'P' then
                    begin
                    qryCR.Edit;
                    qryCRVL_RESTANTE.asFloat := 0.00;
                    qryCRSITUACAO.Value := 'T';
                    if qryCROBS.IsNull or (qryCROBS.Value = '') then
                    qryCROBS.Value := 'Gerada parcela do restante.'
                    else
                    qryCROBS.Value := qryCROBS.Value;
                    ...
                    frmCadReceber.Caption := 'Cadastro de Renegociação';
                    frmCadReceber.cxSair.Enabled := False;
                    ...
                    frmCadReceber.qrycrDTVENCIMENTO.Value := date;
                    frmCadReceber.DBEdit7.ReadOnly := True;
                    frmCadReceber.qrycrFKCLIENTE.Value := qryCRBaixaFKCLIENTE.Value;
                    frmCadReceber.DBLookupComboboxEh4.ReadOnly := True;
                    frmCadReceber.qrycrFKEMPRESA.Value := dados.qryEmpresaCODIGO.Value;
                    frmCadReceber.DBComboBoxEh1.Text := 'CARTEIRA';
                    frmCadReceber.qryCROBS.Value := 'Renegociação de parcela Nº: ' + qrycrCODIGO.asString + ' Vcnt: ' + qryCRDTVENCIMENTO.asString;
                    frmCadReceber.DBEdit3.Text := 'Ref. renegociação';
                    frmCadReceber.qryCRREMESSA.Value := 'N';
                    frmCadReceber.qryCRREMESSA_REENVIAR.Value := 'S';
                    frmCadReceber.qrycrFKCONTA.Value := 0;
                    frmCadReceber.qrycrVALOR.Value := qryBaixaLoteTOTAL_FINAL.asFloat - qryBaixaLoteTOTAL_RECEBIDO.asFloat;
...
procedure TfrmBaixaReceberLote.btnEstornarClick(Sender: TObject);
begin
  if qryBaixaLote.State in dsEditModes then
    qryBaixaLote.Edit;
    qryBaixaLoteJUROS_PERC.AsFloat := 0;
    qryBaixaLoteJUROS_VALOR.AsFloat := 0;
end;
...
begin
  ...
  qryFPG.Close;
  qryFPG.Open;
    DBLookupComboboxEh1.Text := 'RECEBIMENTODE CONTAS';
    DBLookupComboboxEh2.Text := 'CAIXA GERAL';
end;
