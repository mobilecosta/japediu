...
uses Udados, uCadPessoa, uCadProduto, uDMEstoque, uGradeOS, uCadPessoaRapido;
...
procedure TfrmCadOS.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
  if qryOS_Detalhe.State in [dsEdit, dsInsert] then
  begin
    ...
    begin
      ...
      // Excluir o último registro em qryOS_Master
...
var
  sSQL: string;
begin
  Caption := dados.qryParametroTITULO_SISTEMA.Value + ' - Lançamento OS';
  VerificaEstoque := true;
  ...
  vletras := '';
   ...
   case TabControl1.TabIndex of
    0:
      begin
        qryProd.Close;
        sSQL := 'select  CODIGO, DESCRICAO, CODBARRA, REFERENCIA, PR_VENDA, PR_CUSTO, UNIDADE, LOCALIZACAO, SERVICO, QTD_ATUAL  from Produto'
          + ' where SERVICO = ''S'' ' + 'order by descricao';
        qryProd.SQL.Clear;
    ...
    1:
      begin
        qryProd.Close;
        sSQL := 'select  CODIGO, DESCRICAO, CODBARRA, REFERENCIA, PR_VENDA, PR_CUSTO, UNIDADE, LOCALIZACAO, SERVICO, QTD_ATUAL  from Produto'
          + ' where SERVICO = ''N'' ' + 'order by descricao';
        qryProd.SQL.Clear;
        qryProd.SQL.Text := sSQL;
        qryProd.Open;
      end;
      ...
      begin
        ...
        case TabControl1.TabIndex of
          ...
          1:
            begin
              if dados.EhNumero(vletras) and (vletras <> '') then
                qryProd.Filter := 'SERVICO=''N'' AND CODIGO = ' + vletras
              else
              begin

                if dados.qryEmpresaPESQUISA_POR_PARTE.Value = 'S' then
                  qryProd.Filter := 'SERVICO=''N'' AND DESCRICAO LIKE ' +
                    QuotedStr('%' + UpperCase(vletras) + '%');
...
begin
  ...
  begin
    ...
    begin
      ...
      qryOS_MasterDOCUMENTO.AsString := qryClienteCNPJ.AsString;
      qryOS_MasterFONE1.AsString := qryClienteWHATSAPP.AsString;
      qryOS_MasterENDERECO.AsString := qryClienteENDERECO.AsString;
      qryOS_MasterCEP.AsString := qryClienteCEP.AsString;
      qryOS_MasterNUMERO.AsString := qryClienteNUMERO.AsString;
      qryOS_MasterBAIRRO.AsString := qryClienteBAIRRO.AsString;
      qryOS_MasterCIDADE.AsString := qryClienteMUNICIPIO.AsString;
      qryOS_MasterUF.AsString := qryClienteUF.AsString;
    ...
    begin
      ...
      qryOS_MasterENDERECO.Clear;
      qryOS_MasterNUMERO.Clear;
      qryOS_MasterBAIRRO.Clear;
      qryOS_MasterCIDADE.Clear;
      qryOS_MasterUF.Clear;
    end
  end;
...
{$REGION 'Inserindo serviço padrão'}
{$ENDREGION};
end;
...
begin
  try
    ...
    qryOS_Detalhe.FieldByName('FK_PRODUTO').AsInteger :=
      dados.buscacodigoProduto
      ('select codigo from produto where codbarra=:cod or referencia =:ref',
      vPesquisa);
