...
type
  TFrmPDV = class(TForm)
    ...
    ImgFechar: TImage;
    tmrTexto: TTimer;
    pnlMensagemCaixa: TPanel;
    lblMensagemCaixa: TLabel;
    lblDtHr: TLabel;
    DBImage1: TDBImage;
    ...
    MainMenu1: TMainMenu;
    actGerenciarNFCe: TAction;
    dxGerenciamentoNFCe: TdxNavBarItem;
    dxImportar: TdxNavBarItem;
    dxCancelar: TdxNavBarItem;
    Image1: TImage;
    DBImage2: TDBImage;
    Panel21: TPanel;
    ...
    procedure tmrTextoTimer(Sender: TObject);
    procedure TelaDeCaixa(Tipo: Integer; MudaStatusTmrTexto: Boolean = False);
    procedure HabilitaPanel(Habilita: Boolean);
    procedure qryItemAfterClose(DataSet: TDataSet);
    procedure actGerenciarNFCeExecute(Sender: TObject);
    procedure cxButton8Click(Sender: TObject);
    procedure EdtProdutoPClick(Sender: TObject);
    ...
    procedure grpTelefoneClick(Sender: TObject);
 ...
 protected
  ...
  private
    ...
    rEstNormal, rEstFiscal: Extended;
    ...
    procedure WMEnterMenuLoop(var aMessage: TMessage);
    procedure WMExitMenuLoop(var aMessage: TMessage);
    procedure WMMove(var aMessage: TWMMove);
    procedure WMMoving(var aMessage: TMessage);
...
uses Udados, uAbreCaixa, uSuprimento_Sangria,
  ...
  uMenuImportarPDV, uConsVendedor, uGrade, uRemoveProduto, uPessoa, uResumo,
  uChave, uTransfComanda, uConsEntregador, uTef, uDmNFe, uDmPDV, uDesconhecido,
  uCadPessoaRapido, udmImpressao, uReimprimir, uDMEstoque, uLotePDV,
  unit_msg_confirma, uTabelaPrecoPDV, uCadCaixaPDV,
  uCadDevolucao, uPedidoVenda, uNFCe, uSat, UVerAplicacao;


procedure GlobalCEFApp_OnContextInitialized;
begin
  if (FrmPDV <> nil) and FrmPDV.HandleAllocated then
    PostMessage(FrmPDV.Handle, CEFBROWSER_INITIALIZED, 0, 0);
end;
...
begin
  ...
  begin
    result := 0;
    ShowMessage('Serial não foi encontrado!');
  end;
...
procedure TFrmPDV.TelaDeCaixa(Tipo: Integer; MudaStatusTmrTexto: Boolean);
begin
  ...
  if MudaStatusTmrTexto then
    tmrTexto.Enabled := False;
  ...
  if tmrTexto.Enabled then
    Exit;
  HabilitaPanel(True);
  if Tipo = 1 then // Fechado
  begin
    lblCaixa.Caption := ' >> CAIXA FECHADO << ';
    lblMensagemCaixa.Caption := 'CAIXA' + #13 + 'FECHADO';

  end
  else
  begin
    lblCaixa.Caption := ' >> ' + Dados.qryEmpresaFANTASIA.Value +
      ' / SEJA BEM VINDO << ';
    lblMensagemCaixa.Caption := ' >> ' + Dados.qryEmpresaFANTASIA.Value +
      ' / SEJA BEM VINDO << ';
  end;
  lblCaixa.Refresh;
  lblMensagemCaixa.Refresh;
  tmrTexto.Enabled := True;
...
begin
  ...
  { TODO -oWagner Elias -cFunções : Bloqueio do PDv com validade da licença }
  {
...
procedure TFrmPDV.tmrTextoTimer(Sender: TObject);
begin
  lblCaixa.Caption := Copy(lblCaixa.Caption, 2, length(lblCaixa.Caption) - 1) +
    Copy(lblCaixa.Caption, 1, 1);
end;
...
begin
  try
    ...
    qry.close;
    qry.SQL.Clear;
    qry.SQL.Text := 'select sum(vd.vdesconto) desconto from vendas_detalhe vd '
      + 'where (vd.situacao<>''C'') and (vd.fkvenda=' +
      IntToStr(CodVenda) + ')';
    qry.Open;
...
begin
  if dxNavBarPdv.Visible = False then
  begin
    dxNavBarPdv.Visible := False
  end
  else
  begin
    dxNavBarPdv.Visible := False;
  ...
  begin
        ...
        3:
          begin
            ...
            if qryPesqProdVIRTUAL_PRECO.Value > 0 then
              vQtd := vTotal / qryPesqProdVIRTUAL_PRECO.Value;
    ...
    begin
      try
        ...
        ShowMessage('Não foi possível Selecionar Produto!');
      end;
    end;
  end
  else
...
begin
  ...
  try
    ...
    frmCadCaixaPDV.qryCaixaDOC.Value := 'CX. ' +
      IntToStr(qryVendaFK_CAIXA.AsInteger);
...
function TFrmPDV.SearchChromium(aPageIndex : integer; var aChromium : TChromium) : boolean;
var
  i, j : integer;
...
function TFrmPDV.SearchWindowParent(aPageIndex : integer; var aWindowParent : TCEFWindowParent) : boolean;
var
  i, j : integer;
...
procedure TFrmPDV.PageControl1Change(Sender: TObject);
var
...
procedure TFrmPDV.CEFInitializedMsg(var aMessage : TMessage);
begin
...
procedure TFrmPDV.ReloadBtnClick(Sender: TObject);
var
  TempChromium : TChromium;
begin
...
procedure TFrmPDV.RemoveTabBtnClick(Sender: TObject);
var
  TempChromium : TChromium;
begin
...
begin
  ...
  qryPesqProd.ParamByName('CODIGO').AsInteger :=
    StrToInt(Copy(FPesquisa, 1, 6));
...
procedure TFrmPDV.ChamaImg(Img: TImage);
var
  Stm: TStream;
  Jpg: TJPEGImage;
  Bmp: TBitmap;
...
begin
  with Dados.qryEmpresa do
  begin
    if TBlobField(FieldByName('LOGOMARCA')).BlobSize > 0 then
    begin
      Stm := TMemoryStream.Create;
      Jpg := TJPEGImage.Create;
      try
        TBlobField(FieldByName('LOGOMARCA')).SaveToStream(Stm);
        Stm.Position := 0;
        Stm.ReadBuffer(BmpFH, SizeOf(BmpFH));
        Stm.Position := 0;
        if (BmpFH.bfType = $D8FF) then
          Jpg.LoadFromStream(Stm)
        else if (BmpFH.bfType = $4D42) then
        begin
          try
            Bmp := TBitmap.Create;
...
begin
  ...
  begin
    ...
    begin
      ...
      qryVenda.Post;
      Dados.Conexao.Commit;
      ShowMessage('Vendedor: ' + IntToStr(qryVendaFK_VENDEDOR.Value) +
        ' - ' + nomeV);
    end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            Dados.QtdAtualNormalFiscal(qryItemID_PRODUTO.Value, rEstNormal,
              rEstFiscal);

            DMEstoque.AtualizaEstoque(qryItemID_PRODUTO.Value,
              qryItemQTD.AsFloat, 0, 'E', 'R');

            Dados.RegistraHistoricoProduto(qryItemID_PRODUTO.Value, 'PDV',
              rEstNormal, rEstFiscal,
              'Venda: ' + IntToStr(qryVendaCODIGO.AsInteger) +
              ' Cancelada no PDV');
...
begin
  ...
  Begin
    ...
    frmDesconhecido.lblProduto.Caption := qryItemDESCRICAO_SL.Value + ' - R$: '
      + FloatToStr(qryItemPRECO.AsFloat);
...
{$REGION 'Verificar Desconto Maximo'}
    if Dados.qryEmpresaDESCONTO_MAXIMO.AsFloat <
      StrToFloatDef(frmDesconhecido.EdtPorcentagem.Text, 0) then
...
{$ENDREGION}
    ...
    begin
      ...
      Dados.qryExecute.ParamByName('vd').AsFloat :=
        StrToFloatDef(frmDesconhecido.edtPreco.Text, 0);
      Dados.qryExecute.ParamByName('pd').AsFloat :=
        StrToFloatDef(frmDesconhecido.EdtPorcentagem.Text, 0);
      Dados.qryExecute.ParamByName('vl').AsFloat := qryItemTOTAL.AsFloat -
        StrToFloatDef(frmDesconhecido.edtPreco.Text, 0);
      Dados.qryExecute.ParamByName('prod').AsInteger :=
...
begin
  ...
  begin
    if Application.messagebox(PChar('Deseja guardar o LOTE: ' +
      ...
      'Fechamento de Caixa', mb_yesno + MB_ICONQUESTION) = IDYES then
    begin
...
begin
  ...
  begin
    VendaExiste;
    ShowMessage('Venda foi finalizada em outra instancia do sistema!' +
      sLineBreak +
...
procedure TFrmPDV.StopBtnClick(Sender: TObject);
var
  TempChromium : TChromium;
begin
      ...
      begin
        ...
        begin
          if (qryTabPrecoProdPR_VENDA.AsFloat > 0) or
            ...
            (qryTabPrecoProdPR_PRAZO.AsFloat > 0) then
          begin
            try
              ...
              if (qryTabPrecoProdINICIO_PROMOCAO.AsDateTime = now) or
                (qryTabPrecoProdFIM_PROMOCAO.AsDateTime < now) then
                frmTabelaPrecoPDV.cdsTabPrecoVALOR.AsFloat :=
                  qryTabPrecoProdPRECO_PROMO_VAREJO.AsFloat
              else
                frmTabelaPrecoPDV.cdsTabPrecoVALOR.AsFloat :=
                  qryTabPrecoProdPR_VENDA.AsFloat;
              frmTabelaPrecoPDV.cdsTabPrecoIMG.AsInteger := 0;
              frmTabelaPrecoPDV.cdsTabPreco.Post;
              frmTabelaPrecoPDV.cdsTabPreco.Append;
              frmTabelaPrecoPDV.cdsTabPrecoDESCRICAO.AsString := 'CARTÃO';
              frmTabelaPrecoPDV.cdsTabPrecoVALOR.AsFloat :=
                qryTabPrecoProdPR_CARTAO.AsFloat;
              frmTabelaPrecoPDV.cdsTabPrecoIMG.AsInteger := 1;
              frmTabelaPrecoPDV.cdsTabPreco.Post;
              frmTabelaPrecoPDV.cdsTabPreco.Append;
              frmTabelaPrecoPDV.cdsTabPrecoDESCRICAO.AsString := 'PRAZO';
              frmTabelaPrecoPDV.cdsTabPrecoVALOR.AsFloat :=
                qryTabPrecoProdPR_PRAZO.AsFloat;
...
begin
  ...
  begin
    ...
    try
      ...
      AtualizaVendedor;
      dxStatusBar1.Panels[2].Text := 'Vendedor: ' +
        IntToStr(frmconsVendedor.idVendedor) + '-' + frmconsVendedor.vNome;
      frmconsVendedor.Release;
    end;
  end;
...
begin
  try
    ...
    on e: Exception do
      ShowMessage('Balança Habilitada! Verifique a configuração da Balança:' +
...
begin
  ...
  if not Dados.qryMesas.Locate('codigo', numero, []) then
    ShowMessage('Mesa não existe!');

  if Dados.qryMesasSITUACAO.Value = 'O' then
    ShowMessage('Mesa já está ocupada!');
...
begin
  ...
  if Qtd <= 0 then
  begin
    ShowMessage('Produto está com a quantidade inválida 0');
  ...
  if Qtd > 9999 then
  begin
    ShowMessage('Produto está com a quantidade acima do Permitido 9999');
  ...
  try
    ...
    if qryPesqProdPR_CUSTO2.AsFloat > 0 then
      Dados.qryExecute.ParamByName('PR_CUSTO2').AsFloat :=
        qryPesqProdPR_CUSTO2.AsFloat
    else
      Dados.qryExecute.ParamByName('PR_CUSTO2').AsFloat :=
        qryPesqProdPR_CUSTO.AsFloat;
    ...
    begin
      ...
      begin
        ...
        Dados.RegistraHistoricoProduto(qryItemID_PRODUTO.Value, 'PDV',
          rEstNormal, rEstFiscal, 'Baixa Estoque na Venda: ' +
          IntToStr(qryVendaCODIGO.AsInteger) + ' no PDV');
    ...
    on e: Exception do
      ShowMessage(e.Message + sLineBreak + 'Entre em contato com o suporte!');
  end;
...
begin
  try
    ...
    begin
      sVenda := sVenda + qryVenda.Fields[i].FieldName + ': ' +
    ...
    begin
      ...
      begin
        sItem := sItem + qryItem.Fields[i].FieldName + ': ' +
...
procedure TFrmPDV.Chromium_OnAfterCreated(Sender: TObject; const browser: ICefBrowser);
var
...
procedure TFrmPDV.Chromium_OnAddressChange(Sender: TObject; const browser: ICefBrowser; const frame: ICefFrame; const url: ustring);
var
  TempPageIndex : integer;
begin
  if not(FClosing) and
...
function TFrmPDV.GetPageIndex(const aSender : TObject; var aPageIndex : integer) : boolean;
begin
  Result     := False;
  aPageIndex := -1;
  if (aSender <> nil) and
...
procedure TFrmPDV.Chromium_OnTitleChange(Sender: TObject; const browser: ICefBrowser; const title: ustring);
var
  TempPageIndex : integer;
begin
...
procedure TFrmPDV.Chromium_OnClose(Sender: TObject; const browser: ICefBrowser; var aAction : TCefCloseBrowserAction);
var
  TempPageIndex : integer;
begin
  if GetPageIndex(Sender, TempPageIndex) then
...
procedure TFrmPDV.Chromium_OnBeforeClose(Sender: TObject; const browser: ICefBrowser);
var
  TempPageIndex : integer;
begin
  if GetPageIndex(Sender, TempPageIndex) then
...
procedure TFrmPDV.Chromium_OnBeforePopup(Sender: TObject;
  const browser: ICefBrowser; const frame: ICefFrame; const targetUrl,
  targetFrameName: ustring; targetDisposition: TCefWindowOpenDisposition;
  userGesture: Boolean; const popupFeatures: TCefPopupFeatures;
  var windowInfo: TCefWindowInfo; var client: ICefClient;
...
procedure TFrmPDV.AddTabBtnClick(Sender: TObject);
var
  TempSheet        : TTabSheet;
...
begin
  ...
  begin
    ...
    qryAtualizarTotal.SQL.Text :=
      'select * from VENDAS_DETALHE where FkVenda = :Ven';
    ...
    begin
      ...
      begin
        qryPromoGr.close;
        qryPromoGr.Params[0].AsInteger := qryAtualizarTotal.FieldByName
          ('id_produto').AsInteger;
        qryPromoGr.Open();
        ...
        begin
          ...
          begin
            qryPromoFIm.close;
            qryPromoFIm.Params[0].AsInteger := qryAtualizarTotal.FieldByName
              ('id_produto').AsInteger;
            qryPromoFIm.Open();

            if not qryPromoFImFIM_PROMOCAO.IsNull then
            begin
              precoItem := Dados.Conexao.ExecSQLScalar
                ('select pr_venda_prazo from produto where codigo = ?',
            ...
            begin
              precoItem := Dados.Conexao.ExecSQLScalar
                ('select preco_promo_varejo from produto where codigo = ?',
                [qryAtualizarTotal.FieldByName('id_produto').AsInteger]);
            end;
        ...
        begin
          ...
          begin
            ...
            begin
              precoItem := Dados.Conexao.ExecSQLScalar
                ('select pr_venda from produto where codigo = ?',
                [qryAtualizarTotal.FieldByName('id_produto').AsInteger]);
        ...
        begin
          qtdAta := '0';
          qtdAta := VarToStrDef
            (Dados.Conexao.ExecSQLScalar
            ('select qtd_atacado from produto where codigo = ?',
            [qryAtualizarTotal.FieldByName('id_produto').AsInteger]), '0');
          ...
          begin
            ...
            Dados.qryExecute.ParamByName('vl').AsFloat := precoItem *
              qryAtualizarTotal.FieldByName('qtd').AsFloat;
            Dados.qryExecute.ParamByName('tl').AsFloat := precoItem *
              qryAtualizarTotal.FieldByName('qtd').AsFloat;
            Dados.qryExecute.ParamByName('prod').AsInteger :=
              qryAtualizarTotal.FieldByName('id_produto').AsInteger;
            Dados.qryExecute.ParamByName('ven').AsInteger :=
              qryAtualizarTotal.FieldByName('fkvenda').AsInteger;
            Dados.qryExecute.ExecSQL;
          end
          else if qtdAta = '0' then
          begin
            Dados.qryExecute.close;
            Dados.qryExecute.SQL.Text :=
              'update vendas_detalhe set preco = :pre, valor_item = :vl, ' +
        ...
        else if Dados.qryEmpresaDESCONSIDERAR_QUANT_ATACADO.AsString = 'S' then
        begin
          Dados.qryExecute.close;
          Dados.qryExecute.SQL.Text :=
            'update vendas_detalhe set preco = :pre, valor_item = :vl, ' +
...
procedure TFrmPDV.GoBtnClick(Sender: TObject);
var
  TempChromium : TChromium;
begin
...
procedure TFrmPDV.grpTelefoneClick(Sender: TObject);
begin
 ButtonPnl.Enabled := True;
end;
...
begin
  if (length(TiraPontos(edtFone.EditText)) <> 10) and
    (length(TiraPontos(edtFone.EditText)) <> 11) then
  begin
    if pnDelivery.Enabled then
      edtFone.SetFocus;
    ShowMessage('Número de Telefone Inválido! Digite Novamente.');
  end;
end;
...
procedure TFrmPDV.HabilitaPanel(Habilita: Boolean);
var
  DataHora: TDateTime;
begin
  if Habilita then
  begin
    if Dados.qryEmpresaTELA_BOAS_VINDAS.Value = 'S' then
    begin
    if Dados.FTIpoPDV = 'PDV' then
    begin
      pnlMensagemCaixa.Visible := True;
      // Panel1.Visible  :=  False;
      Panel3.Visible := False;
      Panel9.Visible := False;
      dxNavBarPdv.Visible := False;
      lblDtHr.Caption := DateTimeToStr(now);
    end
    end;

    if Dados.FTIpoPDV = 'DELIVERY' then
    begin
      pnlMensagemCaixa.Visible := False;
      // Panel1.Visible  :=  True;
      Panel3.Visible := True;
      qryPesqConta.close;
      qryPesqConta.Params[0].Value := Dados.idUsuario;
...
procedure TFrmPDV.ImgFecharClick(Sender: TObject);
begin
...
procedure TFrmPDV.cxButton8Click(Sender: TObject);
begin
  dxNavBarPdv.Visible := not dxNavBarPdv.Visible;
end;
...
procedure TFrmPDV.qryItemAfterClose(DataSet: TDataSet);
begin
  HabilitaPanel(True);
  tmrTexto.Enabled := True;
  lblCaixa.Caption := ' >> ' + Dados.qryEmpresaFANTASIA.Value +
...
begin
  ...
  begin
    ...
    begin
      ...
      (* VERIFICAR SE A VENDA FOI IMPORTAÇÃO *)
      ...
      Dados.RegistraHistoricoProduto(qryItemID_PRODUTO.Value, 'PDV', rEstNormal,
        rEstFiscal, 'Item Cancelado na Venda: ' +
        IntToStr(qryVendaCODIGO.AsInteger) + ' do PDV');
...
begin
  ...
  begin
    ...
    begin
      ...
      Dados.RegistraHistoricoProduto(qryItemID_PRODUTO.Value, 'PDV', rEstNormal,
        rEstFiscal, 'Insere Item na Venda: ' +
        IntToStr(qryVendaCODIGO.AsInteger) + ' do PDV');
...
begin
  ...
  if qryItemQTD.Value > 9999 then
    ShowMessage('Quantidade maior que o permitido!!');
  ...
  begin
    ...
    begin
      ...
      if Dados.qryConsulta.FieldByName('qtd').AsFloat < Qtd then
        ShowMessage('Quantidade Grade Insuficiente');
    end;
  end;
...
begin
  ...
  begin
    vSql := ' SELECT PRO.CODIGO,' + ' PRO.DESCRICAO,' + ' PRO.CFOP,' +
      ...
      'PRO.PR_VENDA_PRAZO, ' + ' PRO.PRECO_ATACADO, ' + 'PRO.QTD_ATACADO,' +
      ' PRO.QTD_ATUAL,' + ' PRO.QTD_FISCAL,' +
      ...
      + ' FROM PRODUTO PRO' + ' WHERE (PRO.QTD_ATUAL>0) AND (PRO.ATIVO=''S'') ';
    vSql := vSql + '/*where*/';
  end
  else
  begin
...
procedure TFrmPDV.EdtProdutoPClick(Sender: TObject);
begin
  if FrmPDV.btnCaixa.Tag = 1 then
  begin
    ShowMessage('C A I X A   F E C H A D O!' + sLineBreak +
...
var
  MinhaString: string;
  CaractereProcurado, CaractereProcurado2: Char;
  Posicao: Integer;
begin
  MinhaString := EdtProdutoP.Text;
  CaractereProcurado := '*'; // Caractere a ser procurado
  CaractereProcurado2 := 'X';
  Posicao := 0;
  Posicao := Pos(CaractereProcurado, MinhaString);
  if Posicao = 0 then
  begin
    Posicao := Pos(CaractereProcurado2, MinhaString);
  end;
  if (Posicao = 0) and (MinhaString <> '') then
  begin
...
begin
  ...
  begin
    ...
    { if trim(EdtProdutoP.Text) = '' then
      ...
      if qryItem.RecordCount = 0  then
      begin
      ShowMessage('Não existe produtos para finalizar venda!');
      exit;
      end;
      end; }
...
begin
  ...
  (* VERIFICAR SE USA TABELA DE PREÇO *)
  ...
  if qryPesqProdUSA_PROMO_SEMANA.Value = 'S' then
    edtPrecoP.Text := FormatFloat('0.00',
      Dados.ValorPromoSemanaProduto(qryPesqProdCODIGO.AsInteger));
...
procedure TFrmPDV.FormClick(Sender: TObject);
begin
  if FrmPDV.btnCaixa.Tag = 1 then
  begin
    ShowMessage('C A I X A   F E C H A D O!' + sLineBreak +
...
procedure TFrmPDV.CloseAllBrowsers;
var
  i, j, k : integer;
  TempComponent : TComponent;
  TempSheet : TTabSheet;
...
begin
  ...
  PnCabeCaixa.Color := cor1;
  Panel2.Color := cor1;
  dxStatusBar1.Color := cor2;
  // Panel11.Color := cor1;
  // MakeRounded(Panel11);

  Image1.Picture.LoadFromFile(ExtractFilePath(Application.ExeName) + 'pdv.bmp');
...
procedure TFrmPDV.ImgMinimizarClick(Sender: TObject);
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          result := False;
          ShowMessage('Estoque Insuficinete dos Itens deste KIT !');
        end;
      end;
      qryComposicao.Next;
    end;
...
begin
  ...
  begin
    ...
    case valid of
      0:
        begin
          ShowMessage('TimeOut !' + sLineBreak +
      ...
      -2:
        begin
          ShowMessage('Peso Negativo !' + sLineBreak +
            'Verifique a Balança!!!.');
          if PageControl2.ActivePage = TabPDV then
            EdtProdutoP.SetFocus;
          if PageControl2.ActivePage = tabRestaurante then
      ...
      -10:
        begin
          ShowMessage('Sobrepeso!' + sLineBreak +
            'Peso acima do que a Balança Permite!');
          if PageControl2.ActivePage = TabPDV then
            EdtProdutoP.SetFocus;
          if PageControl2.ActivePage = tabRestaurante then
          begin
...
begin
  ...
  try
    ...
    begin
      ...
      frmCadPessoaRapido.DBEdit8.Text := TiraPontos(edtFone.Text);
    end
    else
    begin
...
procedure TFrmPDV.actGerenciarNFCeExecute(Sender: TObject);
begin
  if dxNavBarPdv.Visible = False then
  begin
    dxNavBarPdv.Visible := False
...
begin
  ...
  try
    ...
    if Application.messagebox
      (PChar('Deseja Gravar a Venda Atual e Iniciar uma Nova?'), 'PDV',
      mb_yesno + MB_ICONQUESTION) = IDYES then
    begin
      if not(qryVenda.State in dsEditModes) then
        qryVenda.Edit;
      if not InputQuery('Vendas', 'Observação', vObs) then
...
begin
  ...
  try
    ...
    frmPedidoVenda.maskInicio.Text := DateToStr(date);
    frmPedidoVenda.maskFim.Text := DateToStr(date);
...
begin
  ...
  begin
    ...
    begin
      DMEstoque.AtualizaLoteVenda(qryItemID_PRODUTO.Value,
        qryLoteVendaFK_VENDA.Value, qryLoteVendaFK_VENDA_DETALHE.Value,
        qryLoteVendaFK_LOTE.Value, qryLoteVendaQTD.AsFloat, aMovimento);
      qryLoteVenda.Next;
    end;
  end;
end;
...
begin
  if qryItem.RecordCount <= 0 then
    ShowMessage('Favor adicionar itens nessa venda!');
...
begin
  ...
  try
    ...
    lblMensagemCaixa.Caption := ' >> ' + Dados.qryEmpresaFANTASIA.Value +
      ' / SEJA BEM VINDO << ';
    // Close;
    if lblquantItem.Caption = '0' then
    begin
      HabilitaPanel(True);
      tmrTexto.Enabled := True;
      lblCaixa.Caption := ' >> ' + Dados.qryEmpresaFANTASIA.Value +
...
procedure TFrmPDV.NotifyMoveOrResizeStarted;
var
  i, j : integer;
  TempChromium : TChromium;
begin
...
procedure TFrmPDV.WMMove(var aMessage : TWMMove);
begin
  inherited;
  NotifyMoveOrResizeStarted;
end;
procedure TFrmPDV.WMMoving(var aMessage : TMessage);
begin
  inherited;
  NotifyMoveOrResizeStarted;
end;
procedure TFrmPDV.WMEnterMenuLoop(var aMessage: TMessage);
begin
  inherited;
  if not(FClosing) and (aMessage.wParam = 0) and (GlobalCEFApp <> nil) then
    GlobalCEFApp.OsmodalLoop := True;
end;
procedure TFrmPDV.WMExitMenuLoop(var aMessage: TMessage);
begin
  inherited;
  if not(FClosing) and (aMessage.wParam = 0) and (GlobalCEFApp <> nil) then
