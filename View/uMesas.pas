...
procedure TfrmMesas.bbExcluirClick(Sender: TObject);
var
  i, Qtd: Integer;
  Aviso: WideString;
  MesasOcupadas: string;
begin
  Qtd := StrToIntDef(InputBox('Informe a Quantidade de Mesas a Excluir',
    'Quantidade:', ''), 0);

  if Qtd <= 0 then
  begin
    ShowMessage('Quantidade inválida. Nenhuma mesa foi excluída.');
    Exit;
  end;

  if Qtd > Dados.qryMesas.RecordCount then
  begin
    ShowMessage('A quantidade informada é maior do que o número total de mesas.');
    Exit;
  end;
  ...
  for i := 1 to Qtd do
  begin
    if Dados.qryMesas.FieldByName('SITUACAO').AsString = 'O' then
    begin
      if MesasOcupadas <> '' then
        MesasOcupadas := MesasOcupadas + ', ';
      MesasOcupadas := MesasOcupadas + IntToStr(Dados.qryMesas.FieldByName('CODIGO').AsInteger);
    end;
    Dados.qryMesas.Prior;
  end;

  if Length(MesasOcupadas) > 0 then
  begin
    Aviso := 'Não é possível apagar a(s) mesa(s) ' + MesasOcupadas +
      ' pois está(ão) ocupada(s). Feche a(s) mesa(s) antes de apagar.';
    Application.MessageBox(PWideChar(Aviso), 'Aviso', MB_ICONWARNING);
    Exit;
  end;

  Dados.qryMesas.Last;
  i := 1;
  while not Dados.qryMesas.Bof and (i <= Qtd) do
  begin
    if Dados.qryMesas.FieldByName('SITUACAO').AsString = 'L' then
    begin
      Dados.qryMesas.Delete;
      Inc(i);
    end
    else
      Dados.qryMesas.Prior;
  ...
  if Dados.qryMesas.UpdatesPending then
  begin
    Dados.Conexao.CommitRetaining;
    ShowMessage('Mesas excluídas com sucesso!');
  end;
end;
...
begin
  Dados.qryMesas.IndexFieldNames := 'CODIGO'; // Definir a ordenação em ordem crescente no campo "CODIGO"
  Dados.qryMesas.Insert;
  Dados.qryMesasCODIGO.Value := Dados.Numerador('MESAS', 'CODIGO', 'N', 'N', 'N');
