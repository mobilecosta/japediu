...
type
  TFrmUsuarios = class(TForm)
    ...
    procedure edtConfirmaKeyPress(Sender: TObject; var Key: Char);
...
begin
  ...
  ShowMessage
    ('Feche o sistema, abra novamemte e faça login com o novo usuário!');
...
begin
  Caption := dados.qryParametroTITULO_SISTEMA.Value + ' - Usuários';
...
procedure TFrmUsuarios.DBLookupComboBox1KeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
    selectnext(Sender as twincontrol, true, true);
...
procedure TFrmUsuarios.edtConfirmaKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #13 then
    selectnext(Sender as twincontrol, true, true);
end;
