...
uses
  ...
  frxExportBaseDialog, frxExportXLS, dxSkinDevExpressDarkStyle, cxGraphics,
  cxLookAndFeels,
  ...
  dxSkinWhiteprint, dxSkinXmas2008Blue, TDI;
...
type
  TfrmPessoas = class(TForm)
    ...
    procedure cxMovimentacaoClick(Sender: TObject);
  private
    procedure localiza;
    procedure tamanho;
...
uses Udados, uCadPessoa, uMovimentacoes;
...
begin
  Caption := dados.qryParametroTITULO_SISTEMA.Value + ' - Contatos';
...
procedure TfrmPessoas.cxMovimentacaoClick(Sender: TObject);
begin
  if (dados.qryPessoasCLI.Value <> 'S') and (dados.qryPessoasFORN.Value <> 'S')
  then
  begin
    ShowMessage('Pessoa não é Cliente/Fornecedor.');
    exit;
  end;
  try
    frmMovimentacoes := TfrmMovimentacoes.Create(Application);
    frmMovimentacoes.iCodPessoa := dados.qryPessoasCODIGO.Value;
    frmMovimentacoes.ShowModal;
  finally
    frmMovimentacoes.Release;
  end;
end;
