...
type
  ...
  private
    ...
    procedure tamanho;
    procedure Botoes;
    procedure ImprimeDevolucao58mm;
    procedure ImprimeDevolucao80mm;
...
uses Udados, uCadPessoa, uCadCompra, uSupervisor, uCadDevolucao, uEmail, uFormaPagamento;
...
begin
  ...
  begin
    ImprimeDevolucao58mm;
  end
  else
  begin
    ImprimeDevolucao80mm;
  end;
end;

procedure TfrmDevolucao.ImprimeDevolucao58mm;
begin
 if frmDevolucao.tag <> 5 then
  begin
    if qryDevolucaoSITUACAO.Value <> 'F' then
  ...
  try
    ...
    frxReport.LoadFromFile(ExtractFilePath(Application.ExeName) +
      '\Relatorio\RelDevolucao58mm.fr3');
    frxReport.ShowReport;
  finally
    btnImp.Enabled := true;
  end;
...
procedure TfrmDevolucao.ImprimeDevolucao80mm;
begin
 if frmDevolucao.tag <> 5 then
  begin
    if qryDevolucaoSITUACAO.Value <> 'F' then
...
begin
  if tag = 5 then  // vindo do form = FormaPagamento (PDV)
  begin
   if  qryDevolucaoSITUACAO.AsString <> 'A'  then
   begin
    ShowMessage('Devolução ja finalizada');
    exit;
   end;

   vCreditoTroca := qryDevolucaoTOTAL.AsCurrency;
   vCodDevolucao := qryDevolucaoCODIGO.AsInteger;
//   ShowMessage(CurrToStr(vCreditoTroca));
   close;
  end else
       bbAlterarClick(self);
end;
...
procedure TfrmDevolucao.Botoes;
begin
