object frmParFinanceiro: TfrmParFinanceiro
  Left = 0
  Top = 0
  Caption = 'Par'#226'metros - Resumo Financeiro'
  ClientHeight = 214
  ClientWidth = 375
  Color = clBtnFace
  ...
  object frxReport: TfrxReport
    ...
    ReportOptions.LastChange = 43838.395386388890000000
  ...
  object qryDiario: TFDQuery
    ...
    SQL.Strings = (
        ...
        'select conta, coalesce(entrada,0) entrada, coalesce(saida,0) sai' +
        'da,(entrada-saida) as saldo from ('
      
        'select CO.descricao AS CONTA, SUM(cx.ENTRADA) AS ENTRADA, SUM(cx' +
        '.SAIDA) AS SAIDA  from caixa cx'
      'LEFT join contas CO on co.codigo=cx.fkconta'
      'WHERE'
      
        '(CX.emissao<=:DATA) AND (CX.empresa>=:ID1 AND CX.empresa<=:ID2) ' +
        'AND (co.tipo in ('#39'X'#39','#39'B'#39','#39'C'#39'))'
      'GROUP BY 1'
      'UNION ALL'
      
        'select '#39'CONTAS A RECEBER'#39', SUM(cr.vl_restante) AS ENTRADA, SUM(0' +
        ') AS SAIDA  from creceber cr'
      'WHERE'
      
        '(CR.dtvencimento<=:DATA) AND (CR.fkempresa>=:ID1 AND CR.fkempres' +
        'a<=:ID2)'
      'UNION ALL'
      
        'select '#39'CONTAS A PAGAR'#39', SUM(0) AS ENTRADA, SUM(cp.vl_restante) ' +
        'AS SAIDA  from cpagar cp'
      'WHERE'
      
        '(Cp.dtvencimento<=:DATA) AND (CP.fkempresa>=:ID1 AND CP.fkempres' +
        'a<=:ID2)'
      ')')
    Left = 184
    Top = 61
    ...
    object qryDiarioCONTA: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CONTA'
      Origin = 'CONTA'
      ProviderFlags = []
