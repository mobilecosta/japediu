...
type
  ...
  private
    ...
    procedure CriarCESTs;
//    Procedure AlteracaoCR;
//    Procedure AlteracaoCR_1;
    procedure Atualizando_telas;
    procedure CriarProcedure_Receber;
    Procedure Novas_Tabelas_2022;
    procedure Atualzacao302;
//    procedure Criar_Campos_Posto;
    procedure Criar_campos_Pix;
    procedure CriarSpeed175;
...
uses Udados, uRevisaoConexaoDados;
...
{procedure TfrmExecute.AlteracaoCR;
var
  Local_i: Integer;
begin
  if MenAlteraCr.Text <> '' then
  begin
    MemoSql.Clear;
    for Local_i := 0 to MenAlteraCr.Lines.Count - 1 do
    begin
      MemoSql.Lines.Add(MenAlteraCr.Lines.Strings[Local_i]);
      if pos(';', MenAlteraCr.Lines.Strings[Local_i]) > 0 then
      begin
...
{procedure TfrmExecute.AlteracaoCR_1;
var
  Local_i: Integer;
begin
  if MenCR1.Text <> '' then
  begin
    MemoSql.Clear;
    for Local_i := 0 to MenCR1.Lines.Count - 1 do
    begin
      MemoSql.Lines.Add(MenCR1.Lines.Strings[Local_i]);
      if pos(';', MenCR1.Lines.Strings[Local_i]) > 0 then
      begin
        IBScript.ExecuteScript(MemoSql.Lines);
        dados.Conexao.Commit;
        MemoSql.Clear;
...
{  MemoSql.Clear;
  MemoSql.Text :=
    'ALTER TABLE COMPRA_ITENS ADD VL_OPERACAO COMPUTED BY ((VL_ITEM + VL_IPI + FRETE1 +  SEGURO + DESPESA + VL_ST + VFCP) - DESCONTO);';
  IBScript.ExecuteScript(MemoSql.Lines);
  dados.Conexao.Commit;
...
procedure TfrmExecute.Atualizando_telas;
var
  Local_i: Integer;
begin
{
  if MenNovasTelas.Text <> '' then
  begin
    MemoSql.Clear;
    for Local_i := 0 to MenNovasTelas.Lines.Count - 1 do
    begin
      MemoSql.Lines.Add(MenNovasTelas.Lines.Strings[Local_i]);
      if pos(';', MenNovasTelas.Lines.Strings[Local_i]) > 0 then
      begin
        IBScript.ExecuteScript(MemoSql.Lines);
        dados.Conexao.Commit;
        MemoSql.Clear;
...
procedure TfrmExecute.Atualzacao302;
var
  Local_i: Integer;
begin
{  if MenV302.Text <> '' then
  begin
    MemoSql.Clear;
    for Local_i := 0 to MenV302.Lines.Count - 1 do
    begin
      MemoSql.Lines.Add(MenV302.Lines.Strings[Local_i]);
      if pos(';', MenV302.Lines.Strings[Local_i]) > 0 then
      begin
        IBScript.ExecuteScript(MemoSql.Lines);
        dados.Conexao.Commit;
        MemoSql.Clear;
...
procedure TfrmExecute.CriarCESTs;
var
  Local_i: Integer;
begin
{  try
    qryConsulta.Close;
    qryConsulta.SQL.Clear;
    qryConsulta.SQL.Text  :=  'SELECT COUNT(*) QTDE '+
                              'FROM RDB$RELATIONS '+
                              'WHERE RDB$FLAGS=1 and RDB$RELATION_NAME=''CEST'' ';
    qryConsulta.Open;
    if qryConsulta.Fields[0].Value = 0 then
      begin
        MemoSql.Clear;
        MemoSql.Text := 'CREATE TABLE CEST ( '+
                        '    CEST VARCHAR(7), '+
                        '    NCM VARCHAR(8), '+
    ...
    qryConsulta.Close;
    qryConsulta.SQL.Clear;
    qryConsulta.SQL.Text  :=  'SELECT COUNT(*) QTDE FROM CEST ';
    qryConsulta.Open;
    if qryConsulta.Fields[0].Value = 0 then
      begin
        if MemoNovo.Text <> '' then
          begin
            ...
            for Local_i := 0 to MemoNovo.Lines.Count - 1 do
              begin
                MemoSql.Lines.Add(memCEST.Lines.Strings[Local_i]);
                if pos(';', memCEST.Lines.Strings[Local_i]) > 0 then
                  begin
                    IBScript.ExecuteScript(MemoSql.Lines);
                    dados.Conexao.Commit;
                    MemoSql.Clear;
...
procedure TfrmExecute.CriarProcedure_Receber;
begin
 {  if memProcedure.Text <> '' then
  begin
        IBScript.ExecuteScript(memProcedure.Lines);
        dados.Conexao.Commit;
  end;   }
end;

procedure TfrmExecute.CriarSpeed175;
   var
  i: Integer;
begin
{  MemoSql.Clear;
  for i := 0 to MenSpeed175.Lines.Count - 1 do
  begin
    MemoSql.Lines.Add(MenSpeed175.Lines.Strings[i]);
    if pos(';', MenSpeed175.Lines.Strings[i]) > 0 then
    begin
      IBScript.ExecuteScript(MemoSql.Lines);
      dados.Conexao.Commit;
      MemoSql.Clear;
...
procedure TfrmExecute.Criar_campos_Pix;
begin
{  try
  qryConsulta.Close;
  qryConsulta.SQL.Text := 'select QRCODE_PIX FROM  FORMA_PAGAMENTO ';
  qryConsulta.Open;
  except
   begin
    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE FORMA_PAGAMENTO ADD QRCODE_PIX VARCHAR(200);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;

    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE FORMA_PAGAMENTO ADD CHAVE_PIX VARCHAR(100);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;
   end;
  end;
  ...
  try
    qryConsulta.Close;
    qryConsulta.SQL.Text := 'select PSP_PIX FROM  EMPRESA ';
    qryConsulta.Open;
   except
   begin
    ...
    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE EMPRESA ADD PSP_PIX VARCHAR(50);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;

    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE EMPRESA ADD AMBIENTE_PIX VARCHAR(40);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;
    ...
    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE EMPRESA ADD CLIENTEID_PIX VARCHAR(500);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;

    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE EMPRESA ADD CHAVE_PIX VARCHAR(200);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;
   end;
  ...
  try
  qryConsulta.Close;
  qryConsulta.SQL.Text := 'select CLIENTSECRET_PIX FROM  FORMA_PAGAMENTO ';
  qryConsulta.Open;
  except
   begin
    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE EMPRESA ADD CLIENTSECRET_PIX VARCHAR(500);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;
   end;
  end;
  ...
  try
  qryConsulta.Close;
  qryConsulta.SQL.Text := 'select DATA_INVENTARIO FROM  SPED_H005 ';
  qryConsulta.Open;
  except
   begin
    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE SPED_H005 ADD DATA_INVENTARIO DATE;';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;
   end;
  end;
      ...
      try
  qryConsulta.Close;
  qryConsulta.SQL.Text := 'select UND FROM  SPED_H010 ';
  qryConsulta.Open;
  except
   begin
    MemoSql.Clear;
    MemoSql.Text :=
      'ALTER TABLE SPED_H010 ADD UND VARCHAR(10);';
    IBScript.ExecuteScript(MemoSql.Lines);
    dados.Conexao.Commit;
   end;
  end;
...
procedure TfrmExecute.Criar_Campos_Posto;
var
  i: Integer;
begin
  MemoSql.Clear;
  for i := 0 to MemPosto.Lines.Count - 1 do
  begin
    MemoSql.Lines.Add(MemPosto.Lines.Strings[i]);
    if pos(';', MemPosto.Lines.Strings[i]) > 0 then
    begin
      IBScript.ExecuteScript(MemoSql.Lines);
      dados.Conexao.Commit;
      MemoSql.Clear;
...
procedure TfrmExecute.Novas_Tabelas_2022;
var
  i: Integer;
begin
 { MemoSql.Clear;
  for i := 0 to MemNovasTabelas.Lines.Count - 1 do
  begin
    MemoSql.Lines.Add(MemNovasTabelas.Lines.Strings[i]);
    if pos(';', MemNovasTabelas.Lines.Strings[i]) > 0 then
    begin
      IBScript.ExecuteScript(MemoSql.Lines);
      dados.Conexao.Commit;
      MemoSql.Clear;
