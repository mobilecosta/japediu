...
uses
  ...
  frxDBSet, frxExportBaseDialog, frxExportPDF, ACBrTEFD;
...
type
  ...
  private
    ...
    function BuscaGrade(aGrade: Integer): String;
...
begin
  try
    Dados.qryMesas.DisableControls;
  finally
  end;
...
begin
  ...
  try
    ...
    AtualizaVendedor;
    StatusBar1.Panels[2].Text := 'Vendedor:' +
      qryVendaVIRTUAL_VENDEDOR.AsString;
...
function TFrmPDV.BuscaGrade(aGrade: Integer): String;
var
  qryBuscaGrade: TFDQuery;
begin
  result := '';
  try
    qryBuscaGrade := TFDQuery.Create(self);
    qryBuscaGrade.Connection := Dados.Conexao;

    qryBuscaGrade.close;
    qryBuscaGrade.SQL.Text :=
      'SELECT DESCRICAO FROM PRODUTO_GRADE WHERE CODIGO=:CODIGO';
    qryBuscaGrade.Params[0].Value := aGrade;
    qryBuscaGrade.Open;
    if qryBuscaGrade.RecordCount > 0 then
      result := qryBuscaGrade.FieldByName('DESCRICAO').AsString;
...
begin
  ...
  if Preco <= 0 then
  begin
    raise Exception.Create('Produto está com o preço inválido');
    exit;
  end;

  if Qtd <= 0 then
  begin
    raise Exception.Create('Produto está com a quantidade inválida');
  end;
...
begin
  ...
  try
    ...
    if qryItem.FieldByName('FK_GRADE').AsInteger > 0 then
      SL.Add(BuscaGrade(qryItemFK_GRADE.AsInteger));
  ...
  begin
    ...
    try
      ...
      begin
        ...
        SL.SaveToFile(ExtractFilePath(Application.ExeName) + 'Bar.txt');
  ...
  begin
    ...
    try
      ...
      begin
        ...
        SL.SaveToFile(ExtractFilePath(Application.ExeName) + 'Cozinha.txt');
...
begin
  ...
  try
    ...
    begin
      ...
      if qryItem.FieldByName('FK_GRADE').AsInteger > 0 then
        SL.Add('<c>' + BuscaGrade(qryItemFK_GRADE.AsInteger) + '</c>');

      if trim(qryItem.FieldByName('OBSERVACAO').AsString) <> '' then
        SL.Add('<c>' + qryItem.FieldByName('OBSERVACAO').AsString + '</c>');
...
begin
  ...
  try
    ...
    qryUpdateVenda := TFDQuery.Create(self);
    qryUpdateVenda.Connection := Dados.Conexao;

    qryPesquisa.close;
    qryPesquisa.SQL.Clear;
    qryPesquisa.SQL.Add
      ('select  vd.id_produto, pro.qtd_atacado, pro.preco_atacado, vd.qtd, SUM(vd.qtd) QTD  from vendas_detalhe vd');
    qryPesquisa.SQL.Add('left join produto pro on pro.codigo=vd.id_produto');
    qryPesquisa.SQL.Add('where');
    qryPesquisa.SQL.Add('vd.fkvenda=:vd and vd.id_produto=:id');
    qryPesquisa.SQL.Add('group by 1,2,3,4');
    qryPesquisa.SQL.Add('having sum(vd.qtd)>=pro.qtd_atacado');
    qryPesquisa.SQL.Add('order by 1');
    qryPesquisa.Params[0].Value := venda;
    qryPesquisa.Params[1].Value := produto;
    qryPesquisa.Open;
    ...
    begin
      qryUpdateVenda.close;
      qryUpdateVenda.SQL.Clear;
      qryUpdateVenda.SQL.Text :=
        'update vendas_detalhe set preco=:preco,  valor_item= qtd*:preco where ID_PRODUTO=:ID and FKVENDA=:FK';
      qryUpdateVenda.Params[0].Value := qryPesquisa.Fields[2].AsFloat;
