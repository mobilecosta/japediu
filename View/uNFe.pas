...
type
  TfrmCadNFe = class(TForm)
    ...
    DBEdit45: TDBEdit;
    Label53: TLabel;
    qryItemVIRTUAL_CEST: TStringField;
    Label55: TLabel;
    edtRazao: TSearchBox;
    btnPesquisa: TPngSpeedButton;
    btnExcluir: TPngSpeedButton;
    btnTodos: TPngSpeedButton;
    ...
    Label91: TLabel;
    DBEdit47: TDBEdit;
    qryVendaCNPJ_INTERMEDIARIO: TStringField;
    qryVendaINDINTERMED: TIntegerField;
    qryVendaNOTA_MANUAL: TStringField;
    qryVendaINDPRES: TIntegerField;
    qryVendaIDCADINTTRAN: TStringField;
    qryVendaBASE_FCP: TFMTBCDField;
    qryVendaTOTAL_FCP: TFMTBCDField;
    qryVendaBASE_FCP_ST: TFMTBCDField;
    qryVendaTOTAL_FCP_ST: TFMTBCDField;
    qrySomaBASE_FCP: TFMTBCDField;
    qrySomaVALOR_FCP: TFMTBCDField;
    qrySomaBASE_FCP_ST: TFMTBCDField;
    qrySomaVALOR_FCP_ST: TFMTBCDField;
    qryItemBASE_FCP: TFMTBCDField;
    qryItemALIQUOTA_FCP: TFMTBCDField;
    qryItemVALOR_FCP: TFMTBCDField;
    qryItemBASE_FCP_ST: TFMTBCDField;
    qryItemALIQUOTA_FCP_ST: TFMTBCDField;
    qryItemVALOR_FCP_ST: TFMTBCDField;
    Label85: TLabel;
    DBEdit74: TDBEdit;
    Label57: TLabel;
    DBEdit48: TDBEdit;
    ...
    procedure edtRazaoInvokeSearch(Sender: TObject);
  ...
  private
    ...
    procedure Nota_Produto_Combate_Probeza;
...
const
  ...
  SQLUpdateMaiorItem = ' update nfe_detalhe set                     ' +
    ' frete = frete + :frete,                                        ' +
  ...
  SQLUpdateST = ' update nfe_detalhe set                            ' +
    ' BASE_ICMS_ST = :BASE_ICMS_ST,                                  ' +
    ' ALIQ_ICMS_ST = :ALIQ_ICMS_ST,                                  ' +
    ' VALOR_ICMS_ST = :VALOR_ICMS_ST,                                ' +
    ' VALOR_ICMS_ST_RET = :VALOR_ICMS_ST_RET                         ' +
    ' where                                                          ' +
    ' codigo=:codigo                                                 ';

  SQLUpdateDif = ' update nfe_detalhe set                           ' +
    ' BASE_ICMS = :BASE_ICMS,                                        ' +
    ' PICMSDEFERIDO = :PICMSDEFERIDO,                                ' +
    ' VICMSDEFERIDO = :VICMSDEFERIDO,                                ' +
    ' VALOR_ICMS = :VALOR_ICMS                                       ' +
    ' where                                                          ' +
    ' codigo=:codigo                                                 ';

  SQLUpdateLeiTransparencia =
    ' update nfe_detalhe set                                        ' +
    ' TRIB_IMP = :TRIB_IMP,                                         ' +
    ' TRIB_FED = :TRIB_FED,                                         ' +
    ' TRIB_EST = :TRIB_EST,                                         ' +
    ' TRIB_MUN = :TRIB_MUN                                          ' +
  ...
  SQLUpdateOutros = ' update nfe_detalhe set                      ' +
    ' SEGURO = :SEGURO,                                           ' +
    ' DESCONTO = :DESCONTO,                                       ' +
    ' OUTROS = :OUTROS,                                           ' +
    ' FRETE = :FRETE,                                             ' +
...
uses Udados, ufrmStatus, uImportarNFe, uDadosWeb, uCadProduto, uCadPessoa,
  uCadTransp, udtmCBR, uDmNFe, uDMEstoque, uPesquisaPessoa, uPesquisaPrincipio,
  uAPITelegramMensagem;
...
begin
  ...
  try
    ...
    qryItem.FieldByName('ID_PRODUTO').Value :=
      Dados.buscacodigoProduto('select codigo from produto where codbarra=:cod or referencia =:ref',
      vPesquisa);
    ...
    qryItem.FieldByName('QTD').AsFloat := vQtd;
     ...
     begin
     qryItem.FieldByName('CFOP').Value := qryItemVIRTUAL_CFOPE.Value;
     end
     else
     begin
     qryItem.FieldByName('CFOP').Value := qryItemVIRTUAL_CFOPI.Value;
     end;
    qryItem.FieldByName('CST').Value := qryItemVIRTUAL_CST.Value;
    qryItem.FieldByName('CSOSN').Value := qryItemVIRTUAL_CSOSN.Value;
    qryItem.FieldByName('NCM').Value := qryItemVIRTUAL_NCM.Value;
    qryItem.FieldByName('PRECO').Value := qryItemVIRTUAL_PRECO.AsFloat;
    qryItem.FieldByName('UNIDADE').Value := qryItemVIRTUAL_UN.AsString;
    qryItem.Post;
...
begin
  ...
  qryItemVICMSUFREMET.Value := 0;
   ...
   if qryVendaNOTA_MANUAL.Value = 'S' then
  begin
    qryItemVBCUFDEST.Value := qryItemBASE_ICMS.AsFloat;
    qryItemVFCP.Value := qryItemVIRTUAL_FCP.AsFloat;
    qryItemPICMSUFDEST.Value := Dados.BuscaIcms('', qryVendaVIRTUAL_UF_CLIENTE.AsString,
      qryVendaVIRTUAL_UF_CLIENTE.AsString);
    qryItemPICMSINTER.Value := Dados.BuscaIcms('', Dados.qryEmpresaUF.AsString,
      qryVendaVIRTUAL_UF_CLIENTE.AsString);
    qryItemPICMSINTERPART.Value := Dados.qryEmpresaDIFAL_DESTINO.AsFloat;
    qryItemVFCPUFDEST.Value := RoundABNT(qryItemVBCUFDEST.AsFloat *
      (qryItemVFCP.AsFloat / 100), 2);
    vDifal := RoundABNT(qryItemVBCUFDEST.AsFloat *
      ((qryItemPICMSUFDEST.AsFloat - qryItemPICMSINTER.AsFloat) / 100), 2);
    qryItemVICMSUFDEST.Value  := RoundABNT(vDifal * (Dados.qryEmpresaDIFAL_DESTINO.AsFloat
      / 100), 2);
    qryItemVICMSUFREMET.Value := RoundABNT(vDifal * (Dados.qryEmpresaDIFAL_ORIGEM.AsFloat
      / 100), 2);
  end
  else
  begin
...
//      begin
//        pPercentualDesconto := qryVendaDESCONTO.AsFloat /
//          qryVendaSUBTOTAL.AsFloat;
//        pPercentualSeguro := qryVendaSEGURO.AsFloat / qryVendaSUBTOTAL.AsFloat;
//        pPercentualOutros := qryVendaOUTROS.AsFloat / qryVendaSUBTOTAL.AsFloat;
//        pPercentualFrete := qryVendaFRETE.AsFloat / qryVendaSUBTOTAL.AsFloat;
//      end;
    ...
    if qryVendaNOTA_MANUAL.AsString <> 'S' then
    begin
      with Dados.qryExecute do
      begin
        Close;
        SQL.Clear;
        SQL.Text := SQLUpdateOutros;
        parambyname('SEGURO').AsFloat := RoundABNT(vSeguro, 2);
        parambyname('DESCONTO').AsFloat := RoundABNT(vDesconto, 2);
        parambyname('OUTROS').AsFloat := RoundABNT(vDespesa, 2);
        parambyname('FRETE').AsFloat := RoundABNT(vFrete, 2);
        parambyname('ITEM').AsInteger := Item;
        parambyname('CODIGO').AsInteger := qryItemCODIGO.AsInteger;
        ExecSQL;
      end;
    end;
  {$ENDREGION}
...
//    qryVendaPESOL.AsFloat := qrySomaPESO_L.AsFloat;
    ...
    qryVendaBASE_FCP.AsFloat := qrySomaBASE_FCP.AsFloat;
    qryVendaTOTAL_FCP.AsFloat := qrySomaVALOR_FCP.AsFloat;
    qryVendaBASE_FCP_ST.AsFloat := qrySomaBASE_FCP_ST.AsFloat;
    qryVendaTOTAL_FCP_ST.AsFloat := qrySomaVALOR_FCP_ST.AsFloat;
...
begin
  ...
  begin
    ...
    qryVendaNOME.Clear;
    ShowMessage('Pessoa não foi encontrada!');
  end;
end;
...
begin
  ...
  begin
    try
      ...
      ShowMessage('erro no cadastro de produto' + sLineBreak +
...
procedure TfrmCadNFe.btnTodosClick(Sender: TObject);
begin
  if not qryItem.IsEmpty then
  begin
    if Application.messagebox('Deseja Excluir Todos os Itens?', 'Confirmação',
      mb_yesno) = mrYes then
                ...
                case DmNFe.ACBrNFe.WebServices.Retorno.cStat of
                  ...
                  begin
                    ShowMessage('Retorno:' + DmNFe.ACBrNFe.WebServices.Retorno.
                      ...
                      'Erro:' + e.Message);
                        LogTelegramBot('Retorno Error - NFe',DmNFe.ACBrNFe.WebServices.Retorno.
                        cStat.ToString + ' - ' +
                        DmNFe.ACBrNFe.WebServices.Retorno.xMotivo + sLineBreak +
                        'Erro:' + e.Message
                        ,' - Retaguarda') ;
...
procedure TfrmCadNFe.btnExcluirClick(Sender: TObject);
begin
  if not qryItem.IsEmpty then
  begin
    if Application.messagebox('Deseja Excluir Item?', 'Confirmação', mb_yesno) = mrYes
    then
      qryItem.delete;
  end;
end;
...
procedure TfrmCadNFe.BuscaAvancada;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        qryItemID_PRODUTO.AsInteger := frmPesquisa.vCodigoProduto;

        if frmPesquisa.vDescricao <> '' then
          qryItemDESCRICAO.AsString := frmPesquisa.vDescricao;
...
procedure TfrmCadNFe.BuscaPessoa;
begin
  try
    frmPesquisaPessoa := tfrmPesquisaPessoa.Create(Application);
    frmPesquisaPessoa.ShowModal;
  finally
    if trim(frmPesquisaPessoa.vDescricao) <> '' then
    begin
      if not(qryVenda.State in dsEditModes) then
        qryVenda.Edit;
      qryVendaID_CLIENTE.AsInteger := frmPesquisaPessoa.vCodigoPessoa;
      qryVendaNOME.AsString := frmPesquisaPessoa.vDescricao;

    end;
    frmPesquisaPessoa.Release;
  end;
end;
...
begin
  ...
  begin
    try
      ...
      ShowMessage('Erro no cadastro de transportador');
    end;
  end;
end;
...
procedure TfrmCadNFe.Nota_Produto_Combate_Probeza;
begin
 // Fundo de combate a pobreza
  ...
  if qryItemVALOR_FCP.AsFloat > 0 then
  begin
    Produto.Imposto.ICMS.vBCFCP := qryItemBASE_FCP.AsFloat;
    Produto.Imposto.ICMS.pFCP := qryItemALIQUOTA_FCP.AsFloat;
    Produto.Imposto.ICMS.vFCP := qryItemVALOR_FCP.AsFloat;
  end;
  ...
  if qryItemVALOR_FCP_ST.AsFloat > 0 then
  begin
    Produto.Imposto.ICMS.vBCFCPST := qryItemBASE_FCP_ST.AsFloat;
    Produto.Imposto.ICMS.pFCPST := qryItemALIQUOTA_FCP_ST.AsFloat;
    Produto.Imposto.ICMS.vFCPST := qryItemVALOR_FCP_ST.AsFloat;
  end;
end;
...
begin
  ...
  if Trim(Produto.Prod.xProd) = '' then
  begin
    Dados.qryExecute.Close;
    dados.qryExecute.SQL.Text := 'Select descricao from produto where codigo = :cod';
    dados.qryExecute.ParamByName('cod').AsInteger := qryItemID_PRODUTO.AsInteger;
    dados.qryExecute.Open() ;
    Produto.Prod.xProd := Dados.qryExecute.FieldByName('descricao').AsString;
  end;
...
begin
  ...
  try
    ...
    ShowMessage('Erro no cadastro de destinatário!');
  end;
end;
...
begin
  try
    ...
    ShowMessage('Erro no cadastro do emitente!');
  end;
end;
...
begin
  try
   ...
   case qryVendaINDPRES.AsInteger of
    0:
    ...
    4:
      NotaF.NFe.Ide.indPres := pcEntregaDomicilio;
    5:
      NotaF.NFe.Ide.indPres := pcPresencialForaEstabelecimento;
    6:
      NotaF.NFe.Ide.indPres := pcOutros;
   end;
//    NotaF.NFe.Ide.indPres := pcPresencial;
//    NotaF.NFe.Ide.indIntermed := iiOperacaoSemIntermediador;
      case qryVendaINDINTERMED.AsInteger of
      0:
      ...
      2:
        begin
          NotaF.NFe.Ide.indIntermed := iiOperacaoComIntermediador;
          NotaF.NFe.infIntermed.CNPJ :=
            TiraPontos(qryVendaCNPJ_INTERMEDIARIO.AsString);
          NotaF.NFe.infIntermed.idCadIntTran := qryVendaIDCADINTTRAN.AsString;
        end;
    end;
    ...
    NotaF.NFe.Ide.verProc := 'Cr Vs: 4.0';
    ...
    ShowMessage('Erro na configuração da nota!');
  end;
end;
...
begin
  ...
  begin
    ShowMessage('Já existe lançamento no caixa!');
  end;

  if ExisteRecebimento(qryVendaFKVENDA.AsInteger) then
  begin
    ShowMessage('Operação não permitida!' + sLineBreak +
...
begin
  ...
  AbreCR;
  AbreCaixa;
  edtRazao.Text := qryVendaVIRTUAL_CLIENTE.AsString;
...
begin
  ...
  LogTelegramBot('Cancelamento - NFe',DmNFe.ACBrNFe.WebServices.Retorno.xMotivo
    ,' - Retaguarda') ;
  Close;
end;
...
begin
  ...
  qryVenda.Post;
  Dados.Conexao.Commit;
    LogTelegramBot('Denegada - NFe', copy(DmNFe.ACBrNFe.NotasFiscais.Items[0]
    .NFe.infNFe.Id, 4, 100)
    ,' - Retaguarda') ;
  Close;
end;
...
begin
  ...
  if qryVendaINDPRES.IsNull then
    qryVendaINDPRES.AsInteger := 0;

  if qryVendaTPINTEGRA.IsNull then
    qryVendaTPINTEGRA.AsInteger := 2;

  if qryVendaTPBANDEIRA.IsNull then
    qryVendaTPBANDEIRA.AsInteger := 27;

  if qryVendaINDINTERMED.IsNull then
    qryVendaINDINTERMED.AsInteger := 0;
...
begin
  if ActiveControl = DBEdit38 then
...
begin
  ...
  if dados.qryEmpresaUSAR_NUMERO_INICIAL.Value = 'S' then
    begin
      qryVendaNUMERO.Value := StrToIntDef(Dados.qryEmpresaN_INICIAL_NFE.AsString, 1);
      if not (dados.qryEmpresa.State in [dsEdit, dsInsert]) then
        dados.qryEmpresa.Edit;
      dados.qryEmpresaUSAR_NUMERO_INICIAL.Value :=  'N';
...
procedure TfrmCadNFe.edtRazaoInvokeSearch(Sender: TObject);
begin
 BuscaPessoa;
end;
...
procedure TfrmCadNFe.btnPesquisaClick(Sender: TObject);
begin
  BuscaAvancada;
end;
