...
type
  TfrmCadDevolucao = class(TForm)
    ...
    procedure ImportarVenda(cCodVenda : Integer);
...
uses Udados, uDMEstoque, uTrocaPDV;
...
procedure TfrmCadDevolucao.FormCreate(Sender: TObject);
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Devolução de Venda';
end;
...
procedure TfrmCadDevolucao.ImportarVenda(cCodVenda: Integer);
var
  vitem: Integer;
  qtd, qtdSaldo: Real;
...
begin
  ...
  try
    ...
    qryQtdDevolvida.Params[0].Value := cCodVenda; //DBEdit3.EditText;
    qryQtdDevolvida.Open;

    dados.qryConsulta.Close;
    dados.qryConsulta.SQL.Text :=
      'select codigo FROM vendas_master where codigo=:cod and situacao=''F''';
    dados.qryConsulta.Params[0].Value := cCodVenda; //DBEdit3.EditText;
    ...
    dados.qryConsulta.SQL.Text :=
      ...
      + ' where FKVENDA=:id';
    dados.qryConsulta.Params[0].Value := cCodVenda ;//qryDevolucaoFK_VENDA.Value;
    dados.qryConsulta.Open;
    ...
    qryVenda.Close;
    qryVenda.Params[0].Value := cCodVenda;//qryDevolucaoFK_VENDA.Value;
    qryVenda.Open;

    qryItensV.Close;
    qryItensV.Params[0].Value := cCodVenda;  //qryVendaCODIGO.Value;
    qryItensV.Open;
...
begin
  try
    ...
    if trim(qryDevolucaoTIPO_DEVOLUCAO.AsString) = '' then
   begin
      ShowMessage('Informe o tipo de devolução!');
      exit;
    end;
...
begin
  ImportarVenda(StrToInt(DBEdit3.EditText));
end;
...
procedure TfrmCadDevolucao.Button1Click(Sender: TObject);
begin
  try
   frmTrocaPDV  := TfrmTrocaPDV.Create(Application);
   frmTrocaPDV.ShowModal;
  finally
   frmTrocaPDV.Release;
  end;
