...
type
  TFrmSpedSf = class(TForm)
    ...
    Label7: TLabel;
    dataInventario: TDateTimePicker;
    Label8: TLabel;
    edtCTa: TEdit;
    chkSoInventario: TCheckBox;
...
procedure TFrmSpedSf.SpeedButton1Click(Sender: TObject);
begin
 close;
end;
...
begin
  ...
  if chkSoInventario.Checked then
    DadosSped.ImportaUNidade(strtodate('01/01/1900'), strtodate('01/01/1900'),
      cbEmpresa.KeyValue, dataInventario.Date)
  else if ckInventario.Checked then
  ...
  // importa produtos
  if chkSoInventario.Checked then
    DadosSped.ImportaProduto(strtodate('01/01/1900'), strtodate('01/01/1900'),
      cbEmpresa.KeyValue, 'F', dataInventario.Date)
  else if ckInventario.Checked then
  ...
  if not chkSoInventario.Checked then
  ...
  if (ckInventario.Checked) or (chkSoInventario.Checked) then
    DadosSped.ImportaInventario(dataInventario.Date, cbEmpresa.KeyValue);
  Application.ProcessMessages;
  ...
  begin
    ...
    begin
      ...
      if (dtIni.Date <= strtodate('31/12/2019')) then
        COD_VER := vlVersao112;
      if (dtIni.Date <= strtodate('01/03/2021')) then
        COD_VER := vlVersao113;
      if (dtIni.Date <= strtodate('31/12/2021')) then
        COD_VER := vlVersao114
      else cod_ver := vlVersao115;
    ...
    begin
      ...
      begin
        if DadosSped.qryContador.IsEmpty then
          IND_MOV := imSemDados
        else
        begin
          IND_MOV := imComDados;
      ...
      begin
        ...
        begin
          ...
          if Dados.qrySped_Participante.IsEmpty then
            IND_MOV := imSemDados
          else
          begin
            IND_MOV := imComDados;
      ...
      begin
        ...
        begin
          if Dados.qrySped_Unidade.IsEmpty then
            IND_MOV := imSemDados
          else
          begin
            IND_MOV := imComDados;
      ...
      begin
        ...
        begin
          if Dados.qrySped_Produto.IsEmpty then
            IND_MOV := imSemDados
          else
          begin
            IND_MOV := imComDados;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (chkSoInventario.Checked or ckSemMovimento.Checked) then
      begin
        IND_MOV := imSemDados;
        exit;
      end;
      ...
      begin
        ...
        begin
          ...
          begin
            if Dados.qrySped_C100.IsEmpty then
              IND_MOV := imSemDados
            else
            begin
              IND_MOV := imComDados;
              ...
              NUM_DOC := FormatFloat('00000000',
                Dados.qrySped_C100num_doc.AsInteger);
            ...
            begin
              ...
              begin
                ...
                if Dados.qrySped_C190.IsEmpty then
                  IND_MOV := imSemDados
                else
                begin
                  IND_MOV := imComDados;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            IND_MOV := imComDados;
            DT_INV := Dados.qrySped_H005DATA_INVENTARIO.AsDateTime;
          ...
          begin
            ...
            begin
              ...
              if Dados.qrySped_H010.IsEmpty then
                IND_MOV := imSemDados
              else
              begin
                IND_MOV := imComDados;

                COD_ITEM := Dados.qrySped_H010FK_PRODUTO.AsString;
                UNID := Dados.qrySped_H010UND.Value;
                ...
                begin
                  Dados.qrySped_h020.Close;
                  Dados.qrySped_h020.Params[0].AsInteger :=
                    Dados.qrySped_H010FK_PRODUTO.AsInteger;
                  Dados.qrySped_h020.Open;

                  aliquota := Dados.BuscaIcms('', Dados.qryEmpresauf.AsString,
                    Dados.qryEmpresauf.AsString);
                  baseicms := Dados.qrySped_H010vl_item.AsFloat;

                  CST_ICMS := Dados.qrySped_h020.FieldByName('csosn').AsString;
                  BC_ICMS := RoundABNT(baseicms, 2);
                  VL_ICMS := RoundABNT(aliquota * baseicms, 2) / 100;
                end;
              end;
            end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if DadosSped.qryconfig_Spedind_cart.Value = 'S' then
      begin

        DadosSped.qry1600.Close;
        DadosSped.qry1600.Params[0].AsDate := dtIni.Date;
        DadosSped.qry1600.Params[1].AsDate := dtFim.Date;
        DadosSped.qry1600.Params[2].AsInteger :=
          Dados.qryEmpresaCODIGO.AsInteger;
        DadosSped.qry1600.Open;

        while not DadosSped.qry1600.Eof do
        begin
          with Registro1600New do
          begin
            COD_PART := DadosSped.qry1600CODIGO.AsString;
            TOT_CREDITO := DadosSped.qry1600CREDITO.AsFloat;
            TOT_DEBITO := DadosSped.qry1600DEBITO.AsFloat;
          end;
          DadosSped.qry1600.Next;
        end;
      end;
