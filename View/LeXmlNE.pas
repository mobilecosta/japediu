...
uses Udados, uCadProduto, PesquisaProduto, uCadCompra, uGrupo, uOperacoes;
...
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Importar XML';
end;
...
procedure TFrmXml.BtnPrecoClick(Sender: TObject);
var
 vValorAntigo, vValorNovo : string;
begin

   vValorAntigo := '';
   vValorNovo   := '';

  if not InputQuery('Atualizar Valor','Valor Pesquisa', vValorAntigo) then
  Exit;
  if not InputQuery('Atualizar Valor','Valor Novo', vValorNovo) then
  Exit;
   try
    ...
    begin
      ...
      dados.qryExecute.SQL.Text :=
        'update xml_detail set pr_venda = :valorn where pr_venda=:ant';
      dados.qryExecute.Params[0].Value := StrToFloat(vValorNovo);
      dados.qryExecute.Params[1].Value := StrToFloat(vValorAntigo);
      dados.qryExecute.ExecSQL;
      dados.Conexao.commit;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            qryXMLDetailCFOP.asString := prod.CFOP;
            ...
            begin
              ...
              if qryXMLDetailCFOP_E.IsNull then
              begin
...
begin
  ...
  try
    ...
    if Dados.qryEmpresaCONSIDERAR_NOME.Value = 'S' then
    FrmPesquisaProduto.edtLoc.Text :=
      Copy(qryXMLDetailDESCRICAO_FORNECEDOR.Value, 1, 40)
    else
    FrmPesquisaProduto.edtLoc.Text :=   qryXMLDetailCODBARRA.AsString;
...
procedure TFrmXml.DBGridEh1DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumnEh; State: TGridDrawState);
begin
  If QryXmlDetailCHECAR.Value = 'S' then // condição
...
procedure TFrmXml.DBGridEh1KeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
 if (Key = VK_RETURN) then
    begin
      if DBGridEh1.SelectedIndex = 4 then
        begin
          try
            frmOperacoes  :=  TfrmOperacoes.Create(Application, False, False);
            frmOperacoes.ShowModal;
            case frmOperacoes.Operacao of
              toMultiplicar:
                begin
                  qryXMLDetailQTD_TOTAL.Value := qryXMLDetailEMBALAGEM.Value *
                    qryXMLDetailQUANT_S.Value;
              ...
              toDividir:
                begin
                  qryXMLDetailQTD_TOTAL.AsFloat := qryXMLDetailEMBALAGEM.AsFloat /
                    qryXMLDetailQUANT_S.AsFloat;
                end;
              else
                begin
                  qryXMLDetailQTD_TOTAL.Value := qryXMLDetailEMBALAGEM.Value *
...
begin
  ...
  try
    ...
    qryCadProduto.Params[1].Value :='-1';

    qryCadProduto.Params[2].Value :='-1';
    qryCadProduto.Open;
    ...
    begin
     ...
     qryCadProduto.Params[0].Value := '-1';
     qryCadProduto.Params[1].Value :='-1';
     qryCadProduto.Params[2].Value := Dados.CodigoBarra_Aux(qryXMLDetailCODBARRA.Value);
     qryCadProduto.Open;
    end;
...
begin
  ...
  dados.qryConsulta.Close;
  dados.qryConsulta.SQL.Text :=
    'SELECT CODIGO, CODBARRA, DESCRICAO FROM PRODUTO WHERE CODBARRA=:COD';
  ...
  begin
    if application.messageBox
      (pchar('O produto ' + dados.qryConsulta.FieldByName('DESCRICAO')
      .AsString + sLineBreak +
      ' Está cadastrado com este código de barras. Deseja Vincular a ele?'),
  ...
  try
    ...
    if not(qryXMLDetail.State in dsEditModes) then
      qryXMLDetail.Edit;
    qryXMLDetailPR_VENDA.AsFloat := FrmCadProduto.qryProdutosPR_VENDA.AsFloat;
    qryXMLDetailPR_SUGESTAO.AsFloat :=
      FrmCadProduto.qryProdutosPR_VENDA.AsFloat;
    qryXMLDetail.Post;
...
begin
  if application.messageBox
    ('Deseja cadastrar todos os produtos automaticamente?', 'Confirmação',
    mb_YesNo) <> mrYes then
    exit;
