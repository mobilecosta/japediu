...
begin
  dadosweb.updWeb.close;
  dadosweb.updWeb.SQL.Text :=
    ' update orcamento set codigolocal=:codigolocal where codigo=:codigo';
  dadosweb.updWeb.Params[0].Value := gid;
...
begin
  ...
  begin
    ...
    begin
      ...
      qryOrcamento.FieldByName('CODIGO_WEB').Value := dadosweb.cdsOrcamento.FieldByName
        ('codigo').Value;
      ...
      qryOrcamento.FieldByName('CNPJ').AsString :=
        CalculaCNPJ(dadosweb.cdsOrcamento.FieldByName('TIPO').AsString,
        dadosweb.cdsOrcamento.FieldByName('CNPJ').AsString);

      if
        (qryOrcamento.FieldByName('CNPJ').AsString <> '00000000000') and
        (qryOrcamento.FieldByName('CNPJ').AsString <> '00000000000000')
      then
        begin
        ...
        begin
          qryOrcamento.FieldByName('CLIENTE').Value := 'CONSUMIDOR FINAL';
          qryOrcamento.FieldByName('FK_CLIENTE').Value := 1;
        end;
...
//        ('GID').Value;
      qryOrcamento.Post;
      UpdateOrcamento(qryOrcamento.FieldByName('CODIGO').Value);
      ...
      dados.qryExecute.Params[0].Value := qryOrcamento.FieldByName
        ('CODIGO').Value;
