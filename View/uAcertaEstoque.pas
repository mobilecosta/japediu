...
uses
  ...
  DBLookupEh, frxExportBaseDialog, frxExportXLS, cxGraphics, cxControls,
  cxLookAndFeels, cxLookAndFeelPainters, cxContainer, cxEdit, dxSkinDevExpressDarkStyle,
  dxGDIPlusClasses, cxImage, Vcl.Menus, cxButtons, cxLabel, dxSkinsCore,
  dxSkinOffice2019Colorful, dxSkinOffice2013DarkGray, dxSkinOffice2019DarkGray;
...
type
  TfrmAcerta = class(TForm)
    ...
    procedure localiza_acerto(cCodigo : Integer);
    procedure Localiza_prodC(Const cCOdigo : INteger);
  private
    fProduto: Integer;
...
begin
  qryProdutos.Close;
  qryProdutos.Params[0].Value := cBarra;
  qryProdutos.Params[1].Value := edtBarra.Text + '%';
  qryProdutos.Open;
...
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Ajustes de Estoque';
Label11.Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Ajustes de Estoque';
  vOrdem := ' ASC';
...
begin
  ...
  if tag = 1 then
  begin
    Caption := 'Lista de Ajustes de Estoque - Para criar um novo ajuste clique no botão "Novo"';
  ...
  begin
    Caption := 'Lista de Ajustes de Estoque Fiscal - Para criar um novo ajuste clique no botão "Novo"';
    DBEdit3.DataField := 'VIRTUAL_QTD_FISCAL';
    DBEdit4.DataField := 'QTD_F';
    DBGrid1.Columns[3].FieldName := 'QTD_F';
  end;

  if cOutraTela = true then
...
procedure TfrmAcerta.localiza_acerto(cCodigo: Integer);
begin
  qryAcerta.Close;
  qryAcerta.SQL.Text := 'select ae.*, pro.descricao from acerta_estoque ae ' +
    ' left join produto pro on pro.codigo=ae.fkproduto where data between :data1 and :data2' ;
  qryAcerta.Params[0].AsDate := DataInicial.date;
  qryAcerta.Params[1].AsDate := DataFinal.date;
  qryAcerta.Open;
end;

procedure TfrmAcerta.Localiza_prodC(const cCOdigo: INteger);
begin
  qryProdutos.Close;
  qryProdutos.Params[0].Value := cCOdigo;
  qryProdutos.Params[1].Value := -1;
  qryProdutos.Open;
  if (qryAcerta.State in dsEditModes) then
    qryAcertaFKPRODUTO.Value := qryProdutosCODIGO.Value;
end;
...
procedure TfrmAcerta.CxAtualizarClick(Sender: TObject);
var
  codigo: Integer;
begin
  try
...
procedure TfrmAcerta.CxLimpaClick(Sender: TObject);
begin
  edtloc.Clear;
  localiza;
end;

procedure TfrmAcerta.cxButton_fecharClick(Sender: TObject);
begin
     close;
end;

procedure TfrmAcerta.CxCancelarClick(Sender: TObject);
begin
...
procedure TfrmAcerta.CxGravarClick(Sender: TObject);
var
...
procedure TfrmAcerta.CxImprimirClick(Sender: TObject);
begin
    try
...
begin
  ...
  if qryAcerta.State in [dsInactive] then
  qryAcerta.Open();
