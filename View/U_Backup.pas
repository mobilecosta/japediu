...
uses
  ...
  FireDAC.VCLUI.Wait, FireDAC.Phys.IBWrapper, IniFiles,
...
begin
  try
      ...
      Panel2.Caption    := 'Backup Iniciado - Aguarde para fechar o sistema';
    ...
    try
      ...
      begin
        ...
        Panel2.Color  := clGreen;
        Panel2.Font.Color := clWhite;
        Panel2.Caption:= 'Backup Finalizado - Liberado para fechar o sistema';
...
begin
  ...
  if not DirectoryExists(ExtractFilePath(Application.ExeName) + 'backup') then
    ...
    if not DirectoryExists(ExtractFilePath(Application.ExeName) + 'backup\' + dados.qryEmpresaCNPJ.AsString) then
    CreateDir(ExtractFilePath(Application.ExeName) + 'backup\' + dados.qryEmpresaCNPJ.AsString);
...
//      '\backup\' + FormatDateTime('dd MM hh nn ss', now) + '.fbk'
    Edit2.Text := '\' + copy(ExtractFileDir(Application.ExeName), 4, 10000) +
      '\backup\' + dados.qryEmpresaCNPJ.AsString + '\'+ dados.qryEmpresaFANTASIA.AsString +
       FormatDateTime('-dd-MM-yyyy-hh-nn-ss', now) + '.fbk'
  end
  else
//    Edit2.Text := '\' + FormatDateTime('dd MM hh nn ss', now) + '.fbk';
    Edit2.Text := '\' + dados.qryEmpresaCNPJ.AsString + '\'+ dados.qryEmpresaFANTASIA.AsString +
       FormatDateTime('-dd-MM-yyyy-hh-nn-ss', now) + '.fbk';
end;
...
//    '\backup\' + FormatDateTime('dd MM hh nn ss', now) + '.fbk';
    ...
    sDiretorio:= ExtractFilePath(Application.ExeName) + 'Dados\DADOS.FDB';
  edtbanco.Text := sDiretorio;
