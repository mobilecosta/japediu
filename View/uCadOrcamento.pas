...
type
  TfrmCadOrcamento = class(TForm)
    ...
    GroupBox6: TGroupBox;
    DBEdit59: TDBEdit;
    qryClientesWHATSAPP: TStringField;
...
begin
Caption := Dados.qryParametroTITULO_SISTEMA.Value + ' - Lançamento de Orçamento';
...
begin
  ...
  qryOrcamentoFKVENDEDOR.Value := dados.qryConfigVENDEDOR_PADRAO.AsInteger;
end;
...
begin
  ...
  vCodIten :=  Dados.buscacodigoProduto('select codigo from produto where codbarra=:cod or referencia =:ref',
    vPesquisa);
  ...
  Dados.qryUpdate.ParamByName('FK_PRODUTO').Value :=  vCodIten;
...
begin
  ...
  try
    ...
    // Verificar se o campo TOTAL é menor ou igual a zero
    if qryOrcamento.FieldByName('TOTAL').AsFloat <= 0 then
    begin
      ShowMessage('Não é permitido salvar Orçamento com valor 0!');
      Exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      qryOrcamentoCELULAR.Value := qryClientesWHATSAPP.Value;
    end
    else
    begin
