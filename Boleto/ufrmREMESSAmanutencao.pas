...
begin
  ...
  if dtsDefault.DataSet.FieldByName('IDBANCO').AsInteger <>
    ...
    raise Exception.Create
      ('Banco gerado nesta remessa difere da configuração selecionada!');
  ...
  begin
    ...
    begin
      ...
      try
        ...
        raise Exception.Create('Informe o código do banco!');
      end;
  ...
  try
    ...
    // grava dados da última remessa nos títulos
    qryDefault.Connection.ExecSQL
...
begin
  if Application.MessageBox
    ('Deseja realmente efetuar a exclusão da remessa selecionada?',
    'Confirmação', MB_ICONQUESTION + MB_YESNO + MB_DEFBUTTON2) = ID_YES then
  begin
    try
      qryDefault.Connection.ExecSQL
...
begin
  ...
  begin
    raise Exception.Create('Arquivo de Remessa já foi gerado!' +
